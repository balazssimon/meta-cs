...
namespace MetaDslx.Languages.Soal
{
    ...
    /// </summary>
    public abstract partial class SoalSyntaxTree : LanguageSyntaxTree
    {
        internal static readonly SoalSyntaxTree Dummy = new DummySyntaxTree();
        /// <summary>
        ...
        /// <summary>
        /// Produces a clone of a <see cref="SoalSyntaxNode"/> which will have current syntax tree as its parent.
        /// 
        /// Caller must guarantee that if the same instance of <see cref="SoalSyntaxNode"/> makes multiple calls
        ...
        protected T CloneNodeAsRoot<T>(T node) where T : SoalSyntaxNode
        {
            return SoalSyntaxNode.CloneNodeAsRoot(node, this);
        }
        /// <summary>
        ...
        public static SoalSyntaxTree Create(SoalSyntaxNode root, SoalParseOptions options = null, string path = "", Encoding encoding = null)
        {
            ...
            var directives = root.Kind == SoalSyntaxKind.Main ?
        ...
        internal static SoalSyntaxTree CreateForDebugger(SoalSyntaxNode root, SourceText text, SoalParseOptions options)
        {
            ...
            CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            options = options ?? SoalParseOptions.Default;
            using (var parser = new SoalSyntaxParser(text, options, oldTree: null, changes: null, cancellationToken: cancellationToken))
            {
        ...
        private SyntaxTree WithChanges(SourceText newText, IReadOnlyList<TextChangeRange> changes)
        {
            ...
            using (var parser = new SoalSyntaxParser(newText, this.Options, oldTree?.GetRoot(), changes))
            {
                var compilationUnit = (MainSyntax)parser.ParseMain().CreateRed();
        ...
        #endregion
        internal sealed class DummySyntaxTree : SoalSyntaxTree
        {
            private readonly SoalSyntaxNode _node;
            ...
            {
                return SoalLanguage.Instance.SyntaxFactory.SyntaxTree((LanguageSyntaxNode)root, options: (SoalParseOptions)options, path: FilePath, encoding: null);
            }
        ...
        {
            public DebuggerSyntaxTree(SoalSyntaxNode root, SourceText text, SoalParseOptions options)
                : base(
                    text,
                    text.Encoding,
