...
namespace PilV2
{
    ...
    /// </summary>
    public abstract partial class PilSyntaxTree : LanguageSyntaxTree
    {
        internal static readonly PilSyntaxTree Dummy = new DummySyntaxTree();
        /// <summary>
        /// The options used by the parser to produce the syntax tree.
        /// </summary>
        public new abstract PilParseOptions Options { get; }
        ...
        /// <summary>
        /// Produces a clone of a <see cref="PilSyntaxNode"/> which will have current syntax tree as its parent.
        /// 
        /// Caller must guarantee that if the same instance of <see cref="PilSyntaxNode"/> makes multiple calls
        ...
        protected T CloneNodeAsRoot<T>(T node) where T : PilSyntaxNode
        {
            return PilSyntaxNode.CloneNodeAsRoot(node, this);
        }
        /// <summary>
        /// Gets the root node of the syntax tree.
        /// </summary>
        public new abstract PilSyntaxNode GetRoot(CancellationToken cancellationToken = default(CancellationToken));
        /// <summary>
        /// Gets the root node of the syntax tree if it is already available.
        /// </summary>
        public abstract bool TryGetRoot(out PilSyntaxNode root);
        /// <summary>
        ...
        /// </remarks>
        public new virtual Task<PilSyntaxNode> GetRootAsync(CancellationToken cancellationToken = default(CancellationToken))
        {
            PilSyntaxNode node;
        ...
        /// </summary>
        public static PilSyntaxTree Create(PilSyntaxNode root, PilParseOptions options = null, string path = "", SourceText text = null, Encoding encoding = null, SourceHashAlgorithm checksumAlgorithm = SourceHashAlgorithm.Sha1)
        {
            if (root == null)
            {
                throw new ArgumentNullException(nameof(root));
            }
            var directives = root.Kind == PilSyntaxKind.Main && root is ICompilationUnitRootSyntax compilationUnitRoot ?
            ...
            return new ParsedSyntaxTree(
                ...
                path: path,
                options: options ?? PilParseOptions.Default,
                root: root,
        ...
        internal static PilSyntaxTree CreateForDebugger(PilSyntaxNode root, SourceText text, PilParseOptions options)
        {
        ...
        /// <summary>
        /// <para>
        /// Internal helper for <see cref="PilSyntaxNode"/> class to create a new syntax tree rooted at the given root node.
        /// This method does not create a clone of the given root, but instead preserves it's reference identity.
        /// </para>
        /// <para>NOTE: This method is only intended to be used from <see cref="PilSyntaxNode.SyntaxTree"/> property.</para>
        /// <para>NOTE: Do not use this method elsewhere, instead use <see cref="Create(PilSyntaxNode, CSharpParseOptions, string, Encoding)"/> method for creating a syntax tree.</para>
        /// </summary>
        internal static PilSyntaxTree CreateWithoutClone(PilSyntaxNode root)
        {
            Debug.Assert(root != null);
            return new ParsedSyntaxTree(
                textOpt: null,
                ...
                path: "",
                options: PilParseOptions.Default,
                root: root,
        ...
        /// </summary>
        public static PilSyntaxTree ParseText(
            string text,
            ...
            CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            options = options ?? PilParseOptions.Default;
            using (var parser = new PilSyntaxParser(text, options, oldTree: null, changes: null, cancellationToken: cancellationToken))
            {
        ...
        {
            return new Antlr4CompletionSource(this.GetRoot(cancellationToken), PilParser._ATN).GetTokenSuggestions(position, cancellationToken);
        }
        public new ImmutableArray<PilSyntaxKind> LookupTokens(int position, CancellationToken cancellationToken)
        {
            return this.LookupTokensCore(position, cancellationToken).SelectAsArray(kind => (PilSyntaxKind)kind);
        }
        #endregion
        #region Changes
        /// <summary>
        ...
        private SyntaxTree WithChanges(SourceText newText, IReadOnlyList<TextChangeRange> changes)
        {
            ...
            using (var parser = new PilSyntaxParser(newText, this.Options, oldTree?.GetRoot(), changes))
            {
                var compilationUnit = (MainSyntax)parser.Parse();
        ...
        #endregion
        internal sealed class DummySyntaxTree : PilSyntaxTree
        {
            private readonly PilSyntaxNode _node;
            ...
            public override PilParseOptions Options
            {
                get { return PilParseOptions.Default; }
            }
            ...
            public override PilSyntaxNode GetRoot(CancellationToken cancellationToken)
            {
                return _node;
            }
            public override bool TryGetRoot(out PilSyntaxNode root)
            {
                root = _node;
                return true;
            }
            ...
            {
                return PilLanguage.Instance.SyntaxFactory.SyntaxTree((LanguageSyntaxNode)root, options: (PilParseOptions)options, path: FilePath, encoding: null);
            }
            public override SyntaxTree WithFilePath(string path)
            {
                return PilLanguage.Instance.SyntaxFactory.SyntaxTree(_node, options: this.Options, path: path, encoding: null);
            }
        }
        private class ParsedSyntaxTree : PilSyntaxTree
        {
            private readonly PilParseOptions _options;
            private readonly string _path;
            private readonly PilSyntaxNode _root;
            private readonly bool _hasCompilationUnitRoot;
            private readonly Encoding _encodingOpt;
            private readonly SourceHashAlgorithm _checksumAlgorithm;
            private SourceText _lazyText;
            internal ParsedSyntaxTree(SourceText textOpt, Encoding encodingOpt, SourceHashAlgorithm checksumAlgorithm, string path, PilParseOptions options, PilSyntaxNode root, DirectiveStack directives, bool cloneRoot = true)
            {
                Debug.Assert(root != null);
                ...
                _hasCompilationUnitRoot = root.Kind == PilSyntaxKind.Main;
        ...
        {
            public DebuggerSyntaxTree(PilSyntaxNode root, SourceText text, PilParseOptions options)
                : base(
                    text,
                    text.Encoding,
