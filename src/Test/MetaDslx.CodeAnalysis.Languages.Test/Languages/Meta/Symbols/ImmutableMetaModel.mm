namespace MetaDslx.CodeAnalysis.Languages.Test.Languages.Meta.Symbols
{
	...
	*/
	metamodel MetaX(Uri="http://metadslx.core/1.0"); 
	...
	*/
	...
	abstract class MetaDeclaration : MetaNamedElement
	{
		...
		derived MetaModelX MetaModel;
	}

	[Scope]
	class MetaNamespace : MetaDeclaration
	{
		containment MetaModelX DefinedMetaModel;
		containment list<MetaDeclaration> Declarations;
	}

	association MetaNamespace.Declarations with MetaDeclaration.Namespace;

	class MetaModelX : MetaNamedElement
	{
		string Uri;
		MetaNamespace Namespace;
	}

	association MetaNamespace.DefinedMetaModel with MetaModelX.Namespace;
