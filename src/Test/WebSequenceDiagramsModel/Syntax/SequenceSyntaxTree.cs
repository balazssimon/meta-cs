...
namespace WebSequenceDiagramsModel
{
    ...
    /// </summary>
    public abstract partial class SequenceSyntaxTree : LanguageSyntaxTree
    {
        internal static readonly SequenceSyntaxTree Dummy = new DummySyntaxTree();
        /// <summary>
        /// The options used by the parser to produce the syntax tree.
        /// </summary>
        public new abstract SequenceParseOptions Options { get; }
        ...
        /// <summary>
        /// Produces a clone of a <see cref="SequenceSyntaxNode"/> which will have current syntax tree as its parent.
        /// 
        /// Caller must guarantee that if the same instance of <see cref="SequenceSyntaxNode"/> makes multiple calls
        ...
        protected T CloneNodeAsRoot<T>(T node) where T : SequenceSyntaxNode
        {
            return SequenceSyntaxNode.CloneNodeAsRoot(node, this);
        }
        /// <summary>
        /// Gets the root node of the syntax tree.
        /// </summary>
        public new abstract SequenceSyntaxNode GetRoot(CancellationToken cancellationToken = default(CancellationToken));
        /// <summary>
        /// Gets the root node of the syntax tree if it is already available.
        /// </summary>
        public abstract bool TryGetRoot(out SequenceSyntaxNode root);
        /// <summary>
        ...
        /// </remarks>
        public new virtual Task<SequenceSyntaxNode> GetRootAsync(CancellationToken cancellationToken = default(CancellationToken))
        {
            SequenceSyntaxNode node;
        ...
        /// </summary>
        public static SequenceSyntaxTree Create(SequenceSyntaxNode root, SequenceParseOptions options = null, string path = "", Encoding encoding = null)
        {
            if (root == null)
            {
                throw new ArgumentNullException(nameof(root));
            }
            var directives = root.Kind == SequenceSyntaxKind.Main ?
            ...
            return new ParsedSyntaxTree(
                ...
                path: path,
                options: options ?? SequenceParseOptions.Default,
                root: root,
        ...
        internal static SequenceSyntaxTree CreateForDebugger(SequenceSyntaxNode root, SourceText text, SequenceParseOptions options)
        {
        ...
        /// <summary>
        /// <para>
        /// Internal helper for <see cref="SequenceSyntaxNode"/> class to create a new syntax tree rooted at the given root node.
        /// This method does not create a clone of the given root, but instead preserves it's reference identity.
        /// </para>
        /// <para>NOTE: This method is only intended to be used from <see cref="SequenceSyntaxNode.SyntaxTree"/> property.</para>
        /// <para>NOTE: Do not use this method elsewhere, instead use <see cref="Create(SequenceSyntaxNode, CSharpParseOptions, string, Encoding)"/> method for creating a syntax tree.</para>
        /// </summary>
        internal static SequenceSyntaxTree CreateWithoutClone(SequenceSyntaxNode root)
        {
            Debug.Assert(root != null);
            return new ParsedSyntaxTree(
                textOpt: null,
                ...
                checksumAlgorithm: SourceHashAlgorithm.Sha1,
                path: "",
                options: SequenceParseOptions.Default,
                root: root,
        ...
        /// </summary>
        public static SequenceSyntaxTree ParseText(
            string text,
            ...
            CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            options = options ?? SequenceParseOptions.Default;
            using (var parser = new SequenceSyntaxParser(text, options, oldTree: null, changes: null, cancellationToken: cancellationToken))
            {
        ...
        private SyntaxTree WithChanges(SourceText newText, IReadOnlyList<TextChangeRange> changes)
        {
            ...
            using (var parser = new SequenceSyntaxParser(newText, this.Options, oldTree?.GetRoot(), changes))
            {
                var compilationUnit = (MainSyntax)parser.ParseMain().CreateRed();
        ...
        #endregion
        internal sealed class DummySyntaxTree : SequenceSyntaxTree
        {
            private readonly SequenceSyntaxNode _node;
            ...
            public override SequenceParseOptions Options
            {
                get { return SequenceParseOptions.Default; }
            }
            ...
            public override SequenceSyntaxNode GetRoot(CancellationToken cancellationToken)
            {
                return _node;
            }
            public override bool TryGetRoot(out SequenceSyntaxNode root)
            {
                root = _node;
                return true;
            }
            ...
            {
                return SequenceLanguage.Instance.SyntaxFactory.SyntaxTree((LanguageSyntaxNode)root, options: (SequenceParseOptions)options, path: FilePath, encoding: null);
            }
            public override SyntaxTree WithFilePath(string path)
            {
                return SequenceLanguage.Instance.SyntaxFactory.SyntaxTree(_node, options: this.Options, path: path, encoding: null);
            }
        }
        private class ParsedSyntaxTree : SequenceSyntaxTree
        {
            private readonly SequenceParseOptions _options;
            private readonly string _path;
            private readonly SequenceSyntaxNode _root;
            private readonly bool _hasCompilationUnitRoot;
            private readonly Encoding _encodingOpt;
            private readonly SourceHashAlgorithm _checksumAlgorithm;
            private SourceText _lazyText;
            internal ParsedSyntaxTree(SourceText textOpt, Encoding encodingOpt, SourceHashAlgorithm checksumAlgorithm, string path, SequenceParseOptions options, SequenceSyntaxNode root, DirectiveStack directives, bool cloneRoot = true)
            {
                Debug.Assert(root != null);
                ...
                _hasCompilationUnitRoot = root.Kind == SequenceSyntaxKind.Main;
        ...
        {
            public DebuggerSyntaxTree(SequenceSyntaxNode root, SourceText text, SequenceParseOptions options)
                : base(
                    text,
                    text.Encoding,
