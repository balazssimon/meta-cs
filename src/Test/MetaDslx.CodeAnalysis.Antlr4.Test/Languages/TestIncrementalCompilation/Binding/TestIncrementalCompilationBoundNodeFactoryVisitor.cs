...
{
	// Make sure to keep this in sync with TestIncrementalCompilationIsBindableNodeVisitor
    public class TestIncrementalCompilationBoundNodeFactoryVisitor : BoundNodeFactoryVisitor, ITestIncrementalCompilationSyntaxVisitor<ArrayBuilder<object>, BoundNode>
    {
        public TestIncrementalCompilationBoundNodeFactoryVisitor(BoundTree boundTree)
			: base(boundTree)
        {

        }

        protected BoundNode CreateBoundDocumentation(BoundTree boundTree, ImmutableArray<object> childBoundNodes, LanguageSyntaxNode syntax, bool hasErrors)
        {
            return this.CreateBoundDocumentationCore(boundTree, childBoundNodes, syntax, hasErrors);
        }

        protected virtual BoundNode CreateBoundDocumentationCore(BoundTree boundTree, ImmutableArray<object> childBoundNodes, LanguageSyntaxNode syntax, bool hasErrors)
        {
            return new BoundDocumentation(TestIncrementalCompilationBoundKind.Documentation, boundTree, childBoundNodes, syntax, hasErrors);
        }
		...
		public BoundNode VisitMetamodelDeclaration(MetamodelDeclarationSyntax node, ArrayBuilder<object> childBoundNodesForParent)
		{
			...
			try
			{
				...
				{
					BoundNode resultNode;
					resultNode = this.CreateBoundDocumentation(this.BoundTree, childBoundNodes.ToImmutableAndFree(), syntax: node, hasErrors: false);
					resultNode = this.CreateBoundSymbolDef(this.BoundTree, ImmutableArray.Create<object>(resultNode), type: typeof(MetaModel), syntax: node, hasErrors: false);
			...
			try
			{
				...
				if (node.MetamodelPrefixProperty != null)
				{
					if (state == BoundNodeFactoryState.InParent)
					{
						...
						{
							this.Visit(node.MetamodelPrefixProperty, childBoundNodesForParent);
						}
					}
					else
					{
		...
		public BoundNode VisitMetamodelPrefixProperty(MetamodelPrefixPropertySyntax node, ArrayBuilder<object> childBoundNodesForParent)
		{
			if (node == null || node.IsMissing) return null;
			var state = this.State;
			if (this.State == BoundNodeFactoryState.InParent) this.State = BoundNodeFactoryState.InNode;
			...
			try
			{
				...
				{
					BoundNode resultNode;
					resultNode = this.CreateBoundProperty(this.BoundTree, childBoundNodes.ToImmutableAndFree(), name: "Prefix", syntax: node, hasErrors: false);
					childBoundNodesForParent.Add(resultNode); 
					return resultNode;
				}
				else
		...
		public BoundNode VisitEnumDeclaration(EnumDeclarationSyntax node, ArrayBuilder<object> childBoundNodesForParent)
		{
			...
			try
			{
				...
				else if (state == BoundNodeFactoryState.InNode)
				{
					BoundNode resultNode;
					resultNode = this.CreateBoundDocumentation(this.BoundTree, childBoundNodes.ToImmutableAndFree(), syntax: node, hasErrors: false);
					resultNode = this.CreateBoundSymbolDef(this.BoundTree, ImmutableArray.Create<object>(resultNode), type: typeof(MetaEnum), syntax: node, hasErrors: false);
					childBoundNodesForParent.Add(resultNode); 
					return resultNode;
				}
				else
		...
		public BoundNode VisitEnumValue(EnumValueSyntax node, ArrayBuilder<object> childBoundNodesForParent)
		{
			...
			try
			{
				...
				else if (state == BoundNodeFactoryState.InNode)
				{
					BoundNode resultNode;
					resultNode = this.CreateBoundDocumentation(this.BoundTree, childBoundNodes.ToImmutableAndFree(), syntax: node, hasErrors: false);
					resultNode = this.CreateBoundSymbolDef(this.BoundTree, ImmutableArray.Create<object>(resultNode), type: typeof(MetaEnumLiteral), syntax: node, hasErrors: false);
					childBoundNodesForParent.Add(resultNode); 
					return resultNode;
				}
				else
		...
		public BoundNode VisitClassDeclaration(ClassDeclarationSyntax node, ArrayBuilder<object> childBoundNodesForParent)
		{
			...
			try
			{
				...
				{
					if (node.KAbstract.GetKind() == TestIncrementalCompilationSyntaxKind.KAbstract)
					{
				...
				else if (state == BoundNodeFactoryState.InNode)
				{
					BoundNode resultNode;
					resultNode = this.CreateBoundDocumentation(this.BoundTree, childBoundNodes.ToImmutableAndFree(), syntax: node, hasErrors: false);
					resultNode = this.CreateBoundSymbolDef(this.BoundTree, ImmutableArray.Create<object>(resultNode), type: typeof(MetaClass), syntax: node, hasErrors: false);
					childBoundNodesForParent.Add(resultNode); 
					return resultNode;
				}
				else
		...
		public BoundNode VisitFieldDeclaration(FieldDeclarationSyntax node, ArrayBuilder<object> childBoundNodesForParent)
		{
			...
			try
			{
				...
				else if (state == BoundNodeFactoryState.InNode)
				{
					BoundNode resultNode;
					resultNode = this.CreateBoundDocumentation(this.BoundTree, childBoundNodes.ToImmutableAndFree(), syntax: node, hasErrors: false);
					resultNode = this.CreateBoundSymbolDef(this.BoundTree, ImmutableArray.Create<object>(resultNode), type: typeof(MetaProperty), syntax: node, hasErrors: false);
					childBoundNodesForParent.Add(resultNode); 
					return resultNode;
				}
				else
		...
		public BoundNode VisitFieldModifier(FieldModifierSyntax node, ArrayBuilder<object> childBoundNodesForParent)
		{
			...
			try
			{
				...
				if (state == BoundNodeFactoryState.InNode || (state == BoundNodeFactoryState.InParent && LookupPosition.IsInNode(this.Position, node.FieldModifier)))
				{
					...
					{
						case TestIncrementalCompilationSyntaxKind.KReadonly:
							...
							break;
						case TestIncrementalCompilationSyntaxKind.KLazy:
							...
							break;
						case TestIncrementalCompilationSyntaxKind.KDerived:
							...
							break;
						case TestIncrementalCompilationSyntaxKind.KUnion:
			...
			try
			{
				...
				if (node.ConstValue != null)
				{
					if (state == BoundNodeFactoryState.InParent)
					{
						...
						{
							this.Visit(node.ConstValue, childBoundNodes);
						}
					}
					else
					{
		...
		public BoundNode VisitConstValue(ConstValueSyntax node, ArrayBuilder<object> childBoundNodesForParent)
		{
			if (node == null || node.IsMissing) return null;
			var state = this.State;
			if (this.State == BoundNodeFactoryState.InParent) this.State = BoundNodeFactoryState.InNode;
		...
		public BoundNode VisitClassType(ClassTypeSyntax node, ArrayBuilder<object> childBoundNodesForParent)
		{
			...
			try
			{
				...
				{
					BoundNode resultNode;
					resultNode = this.CreateBoundSymbolUse(this.BoundTree, childBoundNodes.ToImmutableAndFree(), types: ImmutableArray.Create(typeof(MetaClass), typeof(MetaEnum), typeof(MetaConstant)), syntax: node, hasErrors: false);
					childBoundNodesForParent.Add(resultNode); 
					return resultNode;
				}
				else
		...
		public BoundNode VisitCollectionKind(CollectionKindSyntax node, ArrayBuilder<object> childBoundNodesForParent)
		{
			...
			try
			{
				...
				if (state == BoundNodeFactoryState.InNode || (state == BoundNodeFactoryState.InParent && LookupPosition.IsInNode(this.Position, node.CollectionKind)))
				{
					...
					{
						case TestIncrementalCompilationSyntaxKind.KSet:
							BoundNode boundKSet;
							...
							break;
						case TestIncrementalCompilationSyntaxKind.KList:
							...
							break;
						case TestIncrementalCompilationSyntaxKind.KMultiSet:
							...
							break;
						case TestIncrementalCompilationSyntaxKind.KMultiList:
		...
		public BoundNode VisitOperationDeclaration(OperationDeclarationSyntax node, ArrayBuilder<object> childBoundNodesForParent)
		{
			...
			try
			{
				...
				else if (state == BoundNodeFactoryState.InNode)
				{
					BoundNode resultNode;
					resultNode = this.CreateBoundDocumentation(this.BoundTree, childBoundNodes.ToImmutableAndFree(), syntax: node, hasErrors: false);
					resultNode = this.CreateBoundSymbolDef(this.BoundTree, ImmutableArray.Create<object>(resultNode), type: typeof(MetaOperation), syntax: node, hasErrors: false);
					childBoundNodesForParent.Add(resultNode); 
					return resultNode;
				}
				else
		...
		public BoundNode VisitOperationModifier(OperationModifierSyntax node, ArrayBuilder<object> childBoundNodesForParent)
		{
			if (node == null || node.IsMissing) return null;
			var state = this.State;
			if (this.State == BoundNodeFactoryState.InParent) this.State = BoundNodeFactoryState.InNode;
			...
			try
			{
				...
				if (node.OperationModifierBuilder != null)
				{
					if (state == BoundNodeFactoryState.InParent)
					{
						...
						{
							this.Visit(node.OperationModifierBuilder, childBoundNodesForParent);
						}
					}
					else
					{
				...
				if (node.OperationModifierReadonly != null)
				{
					if (state == BoundNodeFactoryState.InParent)
					{
						...
						{
							this.Visit(node.OperationModifierReadonly, childBoundNodesForParent);
						}
					}
					else
					{
		...
		public BoundNode VisitOperationModifierBuilder(OperationModifierBuilderSyntax node, ArrayBuilder<object> childBoundNodesForParent)
		{
			if (node == null || node.IsMissing) return null;
			var state = this.State;
			if (this.State == BoundNodeFactoryState.InParent) this.State = BoundNodeFactoryState.InNode;
			...
			try
			{
				...
				{
					BoundNode resultNode;
					resultNode = this.CreateBoundProperty(this.BoundTree, childBoundNodes.ToImmutableAndFree(), name: "IsBuilder", value: true, syntax: node, hasErrors: false);
					childBoundNodesForParent.Add(resultNode); 
					return resultNode;
				}
				else
		...
		public BoundNode VisitOperationModifierReadonly(OperationModifierReadonlySyntax node, ArrayBuilder<object> childBoundNodesForParent)
		{
			if (node == null || node.IsMissing) return null;
			var state = this.State;
			if (this.State == BoundNodeFactoryState.InParent) this.State = BoundNodeFactoryState.InNode;
			...
			try
			{
				...
				{
					BoundNode resultNode;
					resultNode = this.CreateBoundProperty(this.BoundTree, childBoundNodes.ToImmutableAndFree(), name: "IsReadonly", value: true, syntax: node, hasErrors: false);
					childBoundNodesForParent.Add(resultNode); 
					return resultNode;
				}
				else
