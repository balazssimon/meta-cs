...
namespace MetaDslx.CodeAnalysis.Antlr4Test.Languages.TestIncrementalCompilation
{
    ...
    /// </summary>
    public abstract partial class TestIncrementalCompilationSyntaxTree : LanguageSyntaxTree
    {
        internal static readonly TestIncrementalCompilationSyntaxTree Dummy = new DummySyntaxTree();
        /// <summary>
        /// The options used by the parser to produce the syntax tree.
        /// </summary>
        public new abstract TestIncrementalCompilationParseOptions Options { get; }
        ...
        /// <summary>
        /// Produces a clone of a <see cref="TestIncrementalCompilationSyntaxNode"/> which will have current syntax tree as its parent.
        /// 
        /// Caller must guarantee that if the same instance of <see cref="TestIncrementalCompilationSyntaxNode"/> makes multiple calls
        ...
        protected T CloneNodeAsRoot<T>(T node) where T : TestIncrementalCompilationSyntaxNode
        {
            return TestIncrementalCompilationSyntaxNode.CloneNodeAsRoot(node, this);
        }
        /// <summary>
        /// Gets the root node of the syntax tree.
        /// </summary>
        public new abstract TestIncrementalCompilationSyntaxNode GetRoot(CancellationToken cancellationToken = default(CancellationToken));
        /// <summary>
        /// Gets the root node of the syntax tree if it is already available.
        /// </summary>
        public abstract bool TryGetRoot(out TestIncrementalCompilationSyntaxNode root);
        /// <summary>
        ...
        /// </remarks>
        public new virtual Task<TestIncrementalCompilationSyntaxNode> GetRootAsync(CancellationToken cancellationToken = default(CancellationToken))
        {
            TestIncrementalCompilationSyntaxNode node;
        ...
        /// </summary>
        public static TestIncrementalCompilationSyntaxTree Create(TestIncrementalCompilationSyntaxNode root, TestIncrementalCompilationParseOptions options = null, string path = "", SourceText text = null, Encoding encoding = null, SourceHashAlgorithm checksumAlgorithm = SourceHashAlgorithm.Sha1)
        {
            if (root == null)
            {
                throw new ArgumentNullException(nameof(root));
            }
            var directives = root.Kind == TestIncrementalCompilationSyntaxKind.Main && root is ICompilationUnitRootSyntax compilationUnitRoot ?
            ...
            return new ParsedSyntaxTree(
                ...
                path: path,
                options: options ?? TestIncrementalCompilationParseOptions.Default,
                root: root,
        ...
        internal static TestIncrementalCompilationSyntaxTree CreateForDebugger(TestIncrementalCompilationSyntaxNode root, SourceText text, TestIncrementalCompilationParseOptions options)
        {
        ...
        /// <summary>
        /// <para>
        /// Internal helper for <see cref="TestIncrementalCompilationSyntaxNode"/> class to create a new syntax tree rooted at the given root node.
        /// This method does not create a clone of the given root, but instead preserves it's reference identity.
        /// </para>
        /// <para>NOTE: This method is only intended to be used from <see cref="TestIncrementalCompilationSyntaxNode.SyntaxTree"/> property.</para>
        /// <para>NOTE: Do not use this method elsewhere, instead use <see cref="Create(TestIncrementalCompilationSyntaxNode, CSharpParseOptions, string, Encoding)"/> method for creating a syntax tree.</para>
        /// </summary>
        internal static TestIncrementalCompilationSyntaxTree CreateWithoutClone(TestIncrementalCompilationSyntaxNode root)
        {
            Debug.Assert(root != null);
            return new ParsedSyntaxTree(
                textOpt: null,
                ...
                path: "",
                options: TestIncrementalCompilationParseOptions.Default,
                root: root,
        ...
        /// </summary>
        public static TestIncrementalCompilationSyntaxTree ParseText(
            string text,
            ...
            CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            options = options ?? TestIncrementalCompilationParseOptions.Default;
            using (var parser = new TestIncrementalCompilationSyntaxParser(text, options, oldTree: null, changes: null, cancellationToken: cancellationToken))
            {
        ...
        {
            return new Antlr4CompletionSource(this.GetRoot(cancellationToken), TestIncrementalCompilationParser._ATN).GetTokenSuggestions(position, cancellationToken);
        }
        public new ImmutableArray<TestIncrementalCompilationSyntaxKind> LookupTokens(int position, CancellationToken cancellationToken)
        {
            return this.LookupTokensCore(position, cancellationToken).SelectAsArray(kind => (TestIncrementalCompilationSyntaxKind)kind);
        }
        #endregion
        #region Changes
        /// <summary>
        ...
        private SyntaxTree WithChanges(SourceText newText, IReadOnlyList<TextChangeRange> changes)
        {
            ...
            using (var parser = new TestIncrementalCompilationSyntaxParser(newText, this.Options, oldTree?.GetRoot(), changes))
            {
                var compilationUnit = (MainSyntax)parser.Parse();
        ...
        #endregion
        internal sealed class DummySyntaxTree : TestIncrementalCompilationSyntaxTree
        {
            private readonly TestIncrementalCompilationSyntaxNode _node;
            ...
            public override TestIncrementalCompilationParseOptions Options
            {
                get { return TestIncrementalCompilationParseOptions.Default; }
            }
            ...
            public override TestIncrementalCompilationSyntaxNode GetRoot(CancellationToken cancellationToken)
            {
                return _node;
            }
            public override bool TryGetRoot(out TestIncrementalCompilationSyntaxNode root)
            {
                root = _node;
                return true;
            }
            ...
            {
                return TestIncrementalCompilationLanguage.Instance.SyntaxFactory.SyntaxTree((LanguageSyntaxNode)root, options: (TestIncrementalCompilationParseOptions)options, path: FilePath, encoding: null);
            }
            public override SyntaxTree WithFilePath(string path)
            {
                return TestIncrementalCompilationLanguage.Instance.SyntaxFactory.SyntaxTree(_node, options: this.Options, path: path, encoding: null);
            }
        }
        private class ParsedSyntaxTree : TestIncrementalCompilationSyntaxTree
        {
            private readonly TestIncrementalCompilationParseOptions _options;
            private readonly string _path;
            private readonly TestIncrementalCompilationSyntaxNode _root;
            private readonly bool _hasCompilationUnitRoot;
            private readonly Encoding _encodingOpt;
            private readonly SourceHashAlgorithm _checksumAlgorithm;
            private SourceText _lazyText;
            internal ParsedSyntaxTree(SourceText textOpt, Encoding encodingOpt, SourceHashAlgorithm checksumAlgorithm, string path, TestIncrementalCompilationParseOptions options, TestIncrementalCompilationSyntaxNode root, DirectiveStack directives, bool cloneRoot = true)
            {
                Debug.Assert(root != null);
                ...
                _hasCompilationUnitRoot = root.Kind == TestIncrementalCompilationSyntaxKind.Main;
            ...
            public override SyntaxTree WithRootAndOptions(SyntaxNode root, ParseOptions options)
            {
                ...
                return new ParsedSyntaxTree(
                    ...
                    (TestIncrementalCompilationSyntaxNode)root,
                    this.GetDirectives());
            }
            public override SyntaxTree WithFilePath(string path)
            {
        ...
        {
            public DebuggerSyntaxTree(TestIncrementalCompilationSyntaxNode root, SourceText text, TestIncrementalCompilationParseOptions options)
                : base(
                    text,
                    text.Encoding,
