...
namespace MetaDslx.CodeAnalysis.Antlr4Test.Languages.TestLanguageOne.Binding
{
	...
	{
        protected TestLangOneDeclarationTreeBuilderVisitor(TestLangOneSyntaxTree syntaxTree, string scriptClassName, bool isSubmission)
            : base(syntaxTree, scriptClassName, isSubmission)
        {
        }

        public static RootSingleDeclaration ForTree(
            TestLangOneSyntaxTree syntaxTree,
            string scriptClassName,
            bool isSubmission)
        {
            var builder = new TestLangOneDeclarationTreeBuilderVisitor(syntaxTree, scriptClassName, isSubmission);
