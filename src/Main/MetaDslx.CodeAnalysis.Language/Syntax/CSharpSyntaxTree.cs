...
namespace Microsoft.CodeAnalysis.CSharp
{
    ...
    public abstract partial class CSharpSyntaxTree : SyntaxTree
    {
        ...
        internal CSharpSyntaxNode GetRootNode()
        {
            return (CSharpSyntaxNode)GetRoot();
        }
        ...
        protected DirectiveStack GetDirectives()
        {
            if (!_hasDirectives)
            {
                var stack = ((InternalSyntax.CSharpSyntaxNode)this.GetRoot().Green).ApplyDirectives(default(DirectiveStack));
        ...
        private bool IsPreprocessorSymbolDefined(DirectiveStack directives, string symbolName)
        {
            switch (directives.IsDefined(symbolName))
            {
                ...
                case DefineState.Undefined:
                    return false;
                default:
                    return ((CSharpParseOptions)this.Options).PreprocessorSymbols.Contains(symbolName);
            }
        }
        ...
        internal PragmaWarningState GetPragmaDirectiveWarningState(string id, int position)
        {
            if (_lazyPragmaWarningStateMap == null)
            {
                ...
                Interlocked.CompareExchange(ref _lazyPragmaWarningStateMap, new CSharpPragmaWarningStateMap(this, false), null);
            }
