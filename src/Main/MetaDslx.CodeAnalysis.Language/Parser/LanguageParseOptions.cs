...
namespace Microsoft.CodeAnalysis.CSharp
{
    ...
    public sealed class CSharpParseOptions : ParseOptions, IEquatable<CSharpParseOptions>
    {
        ...
        internal override void ValidateOptions(ArrayBuilder<Diagnostic> builder)
        {
            ...
            {
                builder.Add(LanguageDiagnostic.Create(LanguageErrorCode.ERR_BadLanguageVersion, LanguageVersion.ToString()));
            }
            ...
            if (!PreprocessorSymbols.IsDefaultOrEmpty)
            {
                foreach (var symbol in PreprocessorSymbols)
                {
                    if (symbol == null)
                    {
                        builder.Add(LanguageDiagnostic.Create(LanguageErrorCode.ERR_InvalidPreprocessingSymbol, "null"));
                    }
                    else if (!SyntaxFacts.IsValidIdentifier(symbol))
                    {
                        builder.Add(LanguageDiagnostic.Create(LanguageErrorCode.ERR_InvalidPreprocessingSymbol, symbol));
                    }
                }
            }
        }
