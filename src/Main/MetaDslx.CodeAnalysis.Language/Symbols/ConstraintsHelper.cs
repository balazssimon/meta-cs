...
namespace Microsoft.CodeAnalysis.CSharp.Symbols
{
    ...
    internal static class ConstraintsHelper
    {
            ...
            DiagnosticBag diagnostics)
        {
            ...
            {
                diagnostics.Add(new DiagnosticWithInfo(pair.DiagnosticInfo, pair.TypeParameter.Locations[0]));
            }
                        ...
                        case TypeKind.TypeParameter:
                            {
                                ...
                                if (constraintTypeParameter.ContainingSymbol == typeParameter.ContainingSymbol)
                                {
                                    ...
                                    if (inProgress.ContainsReference(constraintTypeParameter))
                                    {
                                        ...
                                        diagnosticsBuilder.Add(new TypeParameterDiagnosticInfo(constraintTypeParameter, new LanguageDiagnosticInfo(ErrorCode.ERR_CircularConstraint, constraintTypeParameter, typeParameter)));
                                        continue;
                                    }
                                ...
                                if (!inherited && currentCompilation != null && constraintTypeParameter.IsFromCompilation(currentCompilation))
                                {
                                    ...
                                    diagnosticsBuilder.Add(new TypeParameterDiagnosticInfo(typeParameter, new LanguageDiagnosticInfo(errorCode, typeParameter, constraintTypeParameter)));
                                    continue;
                                }
                            }
                            break;
                            ...
                            if (constraintType.IsNullableType())
                            {
                                ...
                                if (underlyingType.TypeKind == TypeKind.TypeParameter)
                                {
                                    ...
                                    if (underlyingTypeParameter.ContainingSymbol == typeParameter.ContainingSymbol)
                                    {
                                        ...
                                        {
                                            // "Circular constraint dependency involving '{0}' and '{1}'"
                                            diagnosticsBuilder.Add(new TypeParameterDiagnosticInfo(underlyingTypeParameter, new LanguageDiagnosticInfo(ErrorCode.ERR_CircularConstraint, underlyingTypeParameter, typeParameter)));
                                            continue;
                                        }
                                    }
                                }
                    ...
                    if (!deducedBaseType.IsErrorType() && !constraintDeducedBase.IsErrorType())
                    {
                        if (!IsEncompassedBy(conversions, deducedBaseType, constraintDeducedBase, ref useSiteDiagnostics))
                        {
                            if (!IsEncompassedBy(conversions, constraintDeducedBase, deducedBaseType, ref useSiteDiagnostics))
                            {
                                ...
                                diagnosticsBuilder.Add(new TypeParameterDiagnosticInfo(typeParameter, new LanguageDiagnosticInfo(ErrorCode.ERR_BaseConstraintConflict, typeParameter, constraintDeducedBase, deducedBaseType)));
                            }
                            else
                            {
        ...
        internal static ImmutableArray<TypeParameterConstraintClause> MakeTypeParameterConstraintsEarly(
            ...
            SyntaxList<CSharpSyntaxNode> constraintClauses,
            Location location,
            DiagnosticBag diagnostics)
        {
            ...
            foreach (var underlyingTuple in underlyingTupleTypeChain)
            {
                ...
                void populateDiagnosticsAndClear(ArrayBuilder<TypeParameterDiagnosticInfo> builder, DiagnosticBag bag)
                {
                    ...
                    foreach (var pair in builder)
                    {
                        ...
                        bag.Add(new DiagnosticWithInfo(pair.DiagnosticInfo, location));
                    }
        ...
        public static bool CheckConstraintsForNonTuple(
            ...
            bool includeNullability,
            SyntaxNode typeSyntax,
            SeparatedSyntaxList<CSharpSyntaxNode> typeArgumentsSyntax, // may be omitted in synthesized invocations
            Compilation currentCompilation,
            ...
            DiagnosticBag diagnostics)
        {
            ...
            foreach (var pair in diagnosticsBuilder)
            {
                ...
                diagnostics.Add(new DiagnosticWithInfo(pair.DiagnosticInfo, location));
            }
...
hasRelatedInterfaces:
            ...
            HashSet<TypeParameterSymbol> ignoreTypeConstraintsDependentOnTypeParametersOpt)
        {
            ...
            if (typeArgument.Type.IsPointerType() || typeArgument.IsRestrictedType() || typeArgument.SpecialType == SpecialType.System_Void)
            {
                ...
                diagnosticsBuilder.Add(new TypeParameterDiagnosticInfo(typeParameter, new LanguageDiagnosticInfo(ErrorCode.ERR_BadTypeArgument, typeArgument.Type)));
                return false;
            }
            ...
            if (typeArgument.IsStatic)
            {
                ...
                diagnosticsBuilder.Add(new TypeParameterDiagnosticInfo(typeParameter, new LanguageDiagnosticInfo(ErrorCode.ERR_GenericArgIsStaticClass, typeArgument.Type)));
                return false;
            }
            ...
            if (typeParameter.HasReferenceTypeConstraint)
            {
                if (!typeArgument.Type.IsReferenceType)
                {
                    ...
                    diagnosticsBuilder.Add(new TypeParameterDiagnosticInfo(typeParameter, new LanguageDiagnosticInfo(ErrorCode.ERR_RefConstraintNotSatisfied, containingSymbol.ConstructedFrom(), typeParameter, typeArgument.Type)));
                    return false;
                }
                ...
                {
                    var diagnostic = new LanguageDiagnosticInfo(ErrorCode.WRN_NullabilityMismatchInTypeParameterReferenceTypeConstraint, containingSymbol.ConstructedFrom(), typeParameter, typeArgument);
            ...
            if (typeParameter.HasUnmanagedTypeConstraint)
            {
                ...
                if (managedKind == ManagedKind.Managed || !typeArgument.Type.IsNonNullableValueType())
                {
                    ...
                    diagnosticsBuilder.Add(new TypeParameterDiagnosticInfo(typeParameter, new LanguageDiagnosticInfo(ErrorCode.ERR_UnmanagedConstraintNotSatisfied, containingSymbol.ConstructedFrom(), typeParameter, typeArgument.Type)));
                    return false;
                }
            ...
            if (typeParameter.HasValueTypeConstraint && !typeArgument.Type.IsNonNullableValueType())
            {
                ...
                diagnosticsBuilder.Add(new TypeParameterDiagnosticInfo(typeParameter, new LanguageDiagnosticInfo(ErrorCode.ERR_ValConstraintNotSatisfied, containingSymbol.ConstructedFrom(), typeParameter, typeArgument.Type)));
                return false;
            }
            ...
            foreach (var constraintType in constraintTypes)
            {
                if (SatisfiesConstraintType(conversions.WithNullability(false), typeArgument, constraintType, ref useSiteDiagnostics))
                {
                    if (includeNullability && nullabilityDiagnosticsBuilderOpt != null)
                    {
                        ...
                        {
                            var diagnostic = new LanguageDiagnosticInfo(ErrorCode.WRN_NullabilityMismatchInTypeParameterConstraint, containingSymbol.ConstructedFrom(), constraintType, typeParameter, typeArgument);
                            nullabilityDiagnosticsBuilderOpt.Add(new TypeParameterDiagnosticInfo(typeParameter, diagnostic));
                        }
                    }
                    continue;
                ...
                diagnosticsBuilder.Add(new TypeParameterDiagnosticInfo(typeParameter, new LanguageDiagnosticInfo(errorCode, containingSymbol.ConstructedFrom(), distinguisher.First, typeParameter, distinguisher.Second)));
                hasError = true;
            }
            ...
            if (typeParameter.HasConstructorConstraint && !SatisfiesConstructorConstraint(typeArgument.Type))
            {
                ...
                diagnosticsBuilder.Add(new TypeParameterDiagnosticInfo(typeParameter, new LanguageDiagnosticInfo(ErrorCode.ERR_NewConstraintNotSatisfied, containingSymbol.ConstructedFrom(), typeParameter, typeArgument.Type)));
                return false;
            }
        ...
        {
            // "Type parameter '{0}' inherits conflicting constraints '{1}' and '{2}'"
            return new TypeParameterDiagnosticInfo(typeParameter, new LanguageDiagnosticInfo(ErrorCode.ERR_BaseConstraintConflict, typeParameter, deducedBase, classConflict ? "class" : "struct"));
        }
