...
namespace Microsoft.CodeAnalysis.CSharp
{
    ...
    internal abstract partial class Symbol : ISymbol, IFormattable
    {
            ...
            if (info != null)
            {
                if (info.Code == (int)ErrorCode.ERR_BogusType)
                {
                    switch (this.Kind)
                    {
                        ...
                        case SymbolKind.Property:
                        case SymbolKind.Event:
                            info = new LanguageDiagnosticInfo(ErrorCode.ERR_BindToBogus, this);
                            break;
                    }
                }
            }
        ...
        internal static void ReportErrorIfHasConstraints(
            SyntaxList<CSharpSyntaxNode> constraintClauses, DiagnosticBag diagnostics)
        {
            if (constraintClauses.Count > 0)
            {
                diagnostics.Add(
                    ErrorCode.ERR_ConstraintOnlyAllowedOnGenericDecl,
                    constraintClauses[0].GetLocation());
            }
        }
