...
namespace Microsoft.CodeAnalysis.CSharp.Symbols.Metadata.PE
{
    ...
    internal abstract class PENamedTypeSymbol : NamedTypeSymbol
    {
            ...
            string emittedNamespaceName)
        {
            ...
            if (mrEx != null)
            {
                result._lazyUseSiteDiagnostic = new LanguageDiagnosticInfo(ErrorCode.ERR_BogusType, result);
            }
            ...
            out bool mangleName)
        {
            ...
            if (makeBad)
            {
                _lazyUseSiteDiagnostic = new LanguageDiagnosticInfo(ErrorCode.ERR_BogusType, this);
            }
        }
            ...
            if (ReferenceEquals(_lazyDeclaredBaseTypeWithNullability, ErrorTypeSymbol.UnknownResultType))
            {
                ...
                if ((object)declaredBase != null)
                {
                    declaredBase = (NamedTypeSymbol)NullableTypeDecoder.TransformType(
                        TypeWithAnnotations.Create(declaredBase), _handle, ContainingPEModule).Type;
                }
            ...
            try
            {
                ...
                if (interfaceImpls.Count > 0)
                {
                    ...
                    foreach (var interfaceImpl in interfaceImpls)
                    {
                        ...
                        typeSymbol = NullableTypeDecoder.TransformType(TypeWithAnnotations.Create(typeSymbol), interfaceImpl, moduleSymbol).Type;
        ...
        {
            var info = new LanguageDiagnosticInfo(ErrorCode.ERR_ImportedCircularBase, declaredBase, type);
        ...
        protected virtual DiagnosticInfo GetUseSiteDiagnosticImpl()
        {
            ...
            if (!MergeUseSiteDiagnostics(ref diagnostic, CalculateUseSiteDiagnostic()))
            {
                ...
                {
                    diagnostic = new LanguageDiagnosticInfo(ErrorCode.ERR_BogusType, this);
                }
