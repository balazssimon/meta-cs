    ...
    internal abstract class PEModuleBuilder : PEModuleBuilder<CSharpCompilation, SourceModuleSymbol, AssemblySymbol, TypeSymbol, NamedTypeSymbol, MethodSymbol, SyntaxNode, NoPia.EmbeddedTypesManager, ModuleCompilationState>
    {
        ...
        private void ValidateReferencedAssembly(AssemblySymbol assembly, AssemblyReference asmRef, DiagnosticBag diagnostics)
        {
                ...
                asmRef.Identity.ContentType != AssemblyContentType.WindowsRuntime)
            {
                ...
                diagnostics.Add(new LanguageDiagnosticInfo(CSharpErrorCode.WRN_ReferencedAssemblyDoesNotHaveStrongName, assembly), NoLocation.Singleton);
            }
            ...
            {
                diagnostics.Add(new LanguageDiagnosticInfo(CSharpErrorCode.WRN_RefCultureMismatch, assembly, refIdentity.CultureName), NoLocation.Singleton);
            }
                ...
                !(refMachine == Machine.I386 && !assembly.Bit32Required))
            {
                    ...
                    machine != refMachine)
                {
                    ...
                    diagnostics.Add(new LanguageDiagnosticInfo(CSharpErrorCode.WRN_ConflictingMachineAssembly, assembly), NoLocation.Singleton);
                }
            }
        ...
        private void ReportExportedTypeNameCollisions(ImmutableArray<Cci.ExportedType> exportedTypes, DiagnosticBag diagnostics)
        {
            ...
            foreach (var exportedType in exportedTypes)
            {
                ...
                if (ContainsTopLevelType(fullEmittedName))
                {
                    ...
                    {
                        diagnostics.Add(CSharpErrorCode.ERR_ExportedTypeConflictsWithDeclaration, NoLocation.Singleton, type, type.ContainingModule);
                    }
                    else
                    {
                        diagnostics.Add(CSharpErrorCode.ERR_ForwardedTypeConflictsWithDeclaration, NoLocation.Singleton, type);
                    }
                ...
                if (exportedNamesMap.TryGetValue(fullEmittedName, out contender))
                {
                    if ((object)type.ContainingAssembly == sourceAssembly)
                    {
                        ...
                        diagnostics.Add(CSharpErrorCode.ERR_ExportedTypesConflict, NoLocation.Singleton, type, type.ContainingModule, contender, contender.ContainingModule);
                    }
                    else if ((object)contender.ContainingAssembly == sourceAssembly)
                    {
                        // Forwarded type conflicts with exported type
                        diagnostics.Add(CSharpErrorCode.ERR_ForwardedTypeConflictsWithExportedType, NoLocation.Singleton, type, type.ContainingAssembly, contender, contender.ContainingModule);
                    }
                    else
                    {
                        // Forwarded type conflicts with another forwarded type
                        diagnostics.Add(CSharpErrorCode.ERR_ForwardedTypesConflict, NoLocation.Singleton, type, type.ContainingAssembly, contender, contender.ContainingAssembly);
                    }
            ...
            bool needDeclaration = false)
        {
            ...
            if (namedTypeSymbol.OriginalDefinition.Kind == SymbolKind.ErrorType)
            {
                ...
                {
                    diagnostics.Add(diagInfo != null ? Diagnostic.Create(diagInfo) : new LanguageDiagnostic(new LanguageDiagnosticInfo(CSharpErrorCode.ERR_BogusType, string.Empty), syntaxNodeOpt == null ? NoLocation.Singleton : syntaxNodeOpt.Location));
                }
        ...
        private void CheckTupleUnderlyingType(NamedTypeSymbol namedTypeSymbol, SyntaxNode syntaxNodeOpt, DiagnosticBag diagnostics)
        {
            ...
            diagnostics.Add(
                new LanguageDiagnostic(
                    new LanguageDiagnosticInfo(CSharpErrorCode.ERR_PredefinedValueTupleTypeMustBeStruct, namedTypeSymbol.MetadataName),
                    location));
        }
