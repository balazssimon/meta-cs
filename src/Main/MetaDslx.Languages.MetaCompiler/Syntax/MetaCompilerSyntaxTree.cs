...
namespace MetaDslx.Languages.MetaCompiler
{
    ...
    /// </summary>
    public abstract partial class MetaCompilerSyntaxTree : LanguageSyntaxTree
    {
        internal static readonly MetaCompilerSyntaxTree Dummy = new DummySyntaxTree();
        /// <summary>
        /// The options used by the parser to produce the syntax tree.
        /// </summary>
        public new abstract MetaCompilerParseOptions Options { get; }
        ...
        /// <summary>
        /// Produces a clone of a <see cref="MetaCompilerSyntaxNode"/> which will have current syntax tree as its parent.
        /// 
        /// Caller must guarantee that if the same instance of <see cref="MetaCompilerSyntaxNode"/> makes multiple calls
        ...
        protected T CloneNodeAsRoot<T>(T node) where T : MetaCompilerSyntaxNode
        {
            return MetaCompilerSyntaxNode.CloneNodeAsRoot(node, this);
        }
        /// <summary>
        /// Gets the root node of the syntax tree.
        /// </summary>
        public new abstract MetaCompilerSyntaxNode GetRoot(CancellationToken cancellationToken = default(CancellationToken));
        /// <summary>
        /// Gets the root node of the syntax tree if it is already available.
        /// </summary>
        public abstract bool TryGetRoot(out MetaCompilerSyntaxNode root);
        /// <summary>
        ...
        /// </remarks>
        public new virtual Task<MetaCompilerSyntaxNode> GetRootAsync(CancellationToken cancellationToken = default(CancellationToken))
        {
            MetaCompilerSyntaxNode node;
        ...
        /// </summary>
        public static MetaCompilerSyntaxTree Create(MetaCompilerSyntaxNode root, MetaCompilerParseOptions options = null, string path = "", SourceText text = null, Encoding encoding = null, SourceHashAlgorithm checksumAlgorithm = SourceHashAlgorithm.Sha1)
        {
            if (root == null)
            {
                throw new ArgumentNullException(nameof(root));
            }
            var directives = root.Kind == MetaCompilerSyntaxKind.Main && root is ICompilationUnitRootSyntax compilationUnitRoot ?
            ...
            return new ParsedSyntaxTree(
                ...
                path: path,
                options: options ?? MetaCompilerParseOptions.Default,
                root: root,
        ...
        internal static MetaCompilerSyntaxTree CreateForDebugger(MetaCompilerSyntaxNode root, SourceText text, MetaCompilerParseOptions options)
        {
        ...
        /// <summary>
        /// <para>
        /// Internal helper for <see cref="MetaCompilerSyntaxNode"/> class to create a new syntax tree rooted at the given root node.
        /// This method does not create a clone of the given root, but instead preserves it's reference identity.
        /// </para>
        /// <para>NOTE: This method is only intended to be used from <see cref="MetaCompilerSyntaxNode.SyntaxTree"/> property.</para>
        /// <para>NOTE: Do not use this method elsewhere, instead use <see cref="Create(MetaCompilerSyntaxNode, CSharpParseOptions, string, Encoding)"/> method for creating a syntax tree.</para>
        /// </summary>
        internal static MetaCompilerSyntaxTree CreateWithoutClone(MetaCompilerSyntaxNode root)
        {
            Debug.Assert(root != null);
            return new ParsedSyntaxTree(
                textOpt: null,
                ...
                path: "",
                options: MetaCompilerParseOptions.Default,
                root: root,
        ...
        /// </summary>
        public static MetaCompilerSyntaxTree ParseText(
            string text,
            ...
            CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            options = options ?? MetaCompilerParseOptions.Default;
            using (var parser = new MetaCompilerSyntaxParser(text, options, oldTree: null, changes: null, cancellationToken: cancellationToken))
            {
        ...
        {
            return new Antlr4CompletionSource(this.GetRoot(cancellationToken), MetaCompilerParser._ATN).GetTokenSuggestions(position, cancellationToken);
        }
        public new ImmutableArray<MetaCompilerSyntaxKind> LookupTokens(int position, CancellationToken cancellationToken)
        {
            return this.LookupTokensCore(position, cancellationToken).SelectAsArray(kind => (MetaCompilerSyntaxKind)kind);
        }
        #endregion
        #region Changes
        /// <summary>
        ...
        private SyntaxTree WithChanges(SourceText newText, IReadOnlyList<TextChangeRange> changes)
        {
            ...
            using (var parser = new MetaCompilerSyntaxParser(newText, this.Options, oldTree?.GetRoot(), changes))
            {
                var compilationUnit = (MainSyntax)parser.Parse();
        ...
        #endregion
        internal sealed class DummySyntaxTree : MetaCompilerSyntaxTree
        {
            private readonly MetaCompilerSyntaxNode _node;
            ...
            public override MetaCompilerParseOptions Options
            {
                get { return MetaCompilerParseOptions.Default; }
            }
            ...
            public override MetaCompilerSyntaxNode GetRoot(CancellationToken cancellationToken)
            {
                return _node;
            }
            public override bool TryGetRoot(out MetaCompilerSyntaxNode root)
            {
                root = _node;
                return true;
            }
            ...
            {
                return MetaCompilerLanguage.Instance.SyntaxFactory.SyntaxTree((LanguageSyntaxNode)root, options: (MetaCompilerParseOptions)options, path: FilePath, encoding: null);
            }
            public override SyntaxTree WithFilePath(string path)
            {
                return MetaCompilerLanguage.Instance.SyntaxFactory.SyntaxTree(_node, options: this.Options, path: path, encoding: null);
            }
        }
        private class ParsedSyntaxTree : MetaCompilerSyntaxTree
        {
            private readonly MetaCompilerParseOptions _options;
            private readonly string _path;
            private readonly MetaCompilerSyntaxNode _root;
            private readonly bool _hasCompilationUnitRoot;
            private readonly Encoding _encodingOpt;
            private readonly SourceHashAlgorithm _checksumAlgorithm;
            private SourceText _lazyText;
            internal ParsedSyntaxTree(SourceText textOpt, Encoding encodingOpt, SourceHashAlgorithm checksumAlgorithm, string path, MetaCompilerParseOptions options, MetaCompilerSyntaxNode root, DirectiveStack directives, bool cloneRoot = true)
            {
                Debug.Assert(root != null);
                ...
                _hasCompilationUnitRoot = root.Kind == MetaCompilerSyntaxKind.Main;
        ...
        {
            public DebuggerSyntaxTree(MetaCompilerSyntaxNode root, SourceText text, MetaCompilerParseOptions options)
                : base(
                    text,
                    text.Encoding,
