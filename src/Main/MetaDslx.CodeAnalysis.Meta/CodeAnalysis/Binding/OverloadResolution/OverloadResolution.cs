    ...
    public sealed partial class OverloadResolution
    {
            ...
            TypeSymbol returnType = null)
        {
            MethodOrPropertyOverloadResolution(
                methods, typeArguments, receiver, arguments, containingType, isStaticContext, result,
                isMethodGroupConversion, allowRefOmittedArguments, ref useSiteDiagnostics, inferWithDynamic,
                allowUnexpandedForm, returnRefKind, returnType);
        }
            ...
            ref HashSet<DiagnosticInfo> useSiteDiagnostics)
        {
            ...
            MethodOrPropertyOverloadResolution(
                indexers, typeArguments, receiverOpt, arguments, containingType, isStaticContext, result, isMethodGroupConversion: false,
            ...
            where TMember : MemberSymbol, IInvocableMember
        {
            ...
            PerformMemberOverloadResolution(
                results, members, typeArguments, receiver, arguments, containingType, isStaticContext, completeResults: false, isMethodGroupConversion,
                returnRefKind, returnType, allowRefOmittedArguments,
            ...
            if (!OverloadResolutionResultIsValid(results, arguments.HasDynamicArgument))
            {
                ...
                result.Clear();
                PerformMemberOverloadResolution(
                    results, members, typeArguments, receiver, arguments, containingType, isStaticContext,
            ...
            where TMember : MemberSymbol
        {
            ...
            for (int i = 0; i < members.Count; i++)
            {
                AddMemberToCandidateSet(
                    members[i], containingType, results, members, typeArguments, receiver, arguments, completeResults,
            ...
            RemoveStaticInstanceMismatches(results, arguments, isStaticContext, receiver);
        ...
        private static Dictionary<NamedTypeSymbol, ArrayBuilder<TMember>> PartitionMembersByContainingType<TMember>(ArrayBuilder<TMember> members) where TMember : MemberSymbol
        {
        ...
        private static void ClearContainingTypeMap<TMember>(ref Dictionary<NamedTypeSymbol, ArrayBuilder<TMember>> containingTypeMapOpt) where TMember : MemberSymbol
        {
            ...
            where TMember : MemberSymbol
        {
            ...
            var leastOverriddenMember = (TMember)member.GetLeastOverriddenMember(containingType);
        ...
        private static bool IsMoreDerivedOverride(
            MemberSymbol member,
            MemberSymbol moreDerivedOverride,
            ...
            where TMember : MemberSymbol
        {
            ...
            foreach (var otherMember in members)
            {
                ...
                if (otherMember.HidesBaseMembersByName && otherContainingType.IsDerivedFrom(memberContainingType, TypeCompareKind.ConsiderEverything, useSiteDiagnostics: ref useSiteDiagnostics))
                {
                    return true;
                }
            }
            ...
            where TMember : MemberSymbol
        {
            ...
            {
                var result = results[f];
                if (result.Result.IsValid && !TypeArgumentsAccessible(GetTypeArguments(result.Member), ref useSiteDiagnostics))
                {
            ...
            where TMember : MemberSymbol
        {
            ...
            for (int f = 0; f < results.Count; ++f)
            {
                ...
                if (currentType.IsDerivedFrom(type, TypeCompareKind.ConsiderEverything, useSiteDiagnostics: ref useSiteDiagnostics))
                {
                    return true;
                }
            }
            ...
            where TMember : MemberSymbol
        {
            ...
            for (int f = 0; f < results.Count; f++)
            {
                ...
                if (member.ContainingType is InterfaceTypeSymbol)
                {
        ...
        private TypeSymbol GetParameterType(ParameterSymbol parameter, MemberAnalysisResult result)
        {
            ...
            if (result.Kind == MemberResolutionKind.ApplicableInExpandedForm &&
                parameter.IsVarArg && type is ArrayTypeSymbol)
            {
        ...
        private BetterResult BetterFunctionMember<TMember>(
            ...
            ArrayBuilder<ArgumentSymbol> arguments,
            ref HashSet<DiagnosticInfo> useSiteDiagnostics)
            where TMember : MemberSymbol
        {
            ...
            where TMember : MemberSymbol
        {
            ...
            for (i = 0; i < arguments.Count; ++i)
            {
                ...
                if (arguments[i].IsArgList)
                {
                    Debug.Assert(i == arguments.Count - 1);
                    Debug.Assert(m1.Member is MethodLikeSymbol m1l && m1l.IsVarArg && m2.Member is MethodLikeSymbol m2l && m2l.IsVarArg);
                    continue;
                }
                ...
                {
                    if (allSame && Conversions.ClassifyImplicitConversionFromType(type1Normalized, type2Normalized, ref useSiteDiagnostics) != Conversions.Identity)
                    {
                        allSame = false;
                    }
                ...
                if (Conversions.ClassifyImplicitConversionFromType(type1Normalized, type2Normalized, ref useSiteDiagnostics) != Conversions.Identity)
                {
                    allSame = false;
                }
            ...
            if (m1.Member.ContainingType is SubmissionSymbol && m2.Member.ContainingType is SubmissionSymbol)
            {
            ...
            int m1ModifierCount = CustomModifierCount(m1.LeastOverriddenMember);
            int m2ModifierCount = CustomModifierCount(m2.LeastOverriddenMember);
        ...
        private static BetterResult MoreSpecificType(TypeSymbol t1, TypeSymbol t2, ref HashSet<DiagnosticInfo> useSiteDiagnostics)
        {
            ...
            var t1IsTypeParameter = t1 is TypeParameterSymbol;
            var t2IsTypeParameter = t2 is TypeParameterSymbol;
            ...
            out bool okToDowngradeToNeither)
        {
            ...
            if (considerRefKinds)
            {
                ...
                if (refKind1 != refKind2)
                {
                    ...
                    {
                        return conv1 == Conversions.Identity ? BetterResult.Left : BetterResult.Neither;
                    }
                    else
                    {
                        return conv2 == Conversions.Identity ? BetterResult.Right : BetterResult.Neither;
                    }
                }
        ...
        private BetterResult BetterConversionFromExpression(ExpressionSymbol expression, RefKind refKind, TypeSymbol t1, Conversion conv1, TypeSymbol t2, Conversion conv2, ref HashSet<DiagnosticInfo> useSiteDiagnostics, out bool okToDowngradeToNeither)
        {
            ...
            if (conv1 != Conversions.ConditionalExpression && conv2 == Conversions.ConditionalExpression)
                return BetterResult.Left;
            if (conv2 != Conversions.ConditionalExpression && conv1 == Conversions.ConditionalExpression)
                return BetterResult.Right;
        ...
        private bool ExpressionMatchExactly(ExpressionSymbol expression, TypeSymbol t, ref HashSet<DiagnosticInfo> useSiteDiagnostics)
        {
            ...
            if (expression is TupleExpressionSymbol tuple)
            {
                ...
                return ExpressionMatchExactly(tuple, t, ref useSiteDiagnostics);
            }
        ...
        private bool ExpressionMatchExactly(TupleExpressionSymbol tupleSource, TypeSymbol targetType, ref HashSet<DiagnosticInfo> useSiteDiagnostics)
        {
            if (targetType is not TupleTypeSymbol destination)
            {
        ...
        /*
        private class ReturnStatements : Symbols.SymbolVisitor
        {
            ...
            protected override ExpressionSymbol VisitExpressionWithoutStackGuard(ExpressionSymbol node)
            {
        ...
        */
            ...
            int betterConversionTargetRecursionLimit)
        {
            ...
            var lambdaOpt = node as LambdaExpressionSymbol;
            if (lambdaOpt is not null && !lambdaOpt.IsBound) lambdaOpt = null;
            ...
            if (type1ToType2)
            {
                ...
                okToDowngradeToNeither = lambdaOpt is not null && CanDowngradeConversionFromLambdaToNeither(BetterResult.Left, lambdaOpt, type1, type2, ref useSiteDiagnostics, true);
                return BetterResult.Left;
            }
            else if (type2ToType1)
            {
                // - An implicit conversion from T1 to T2 exists
                okToDowngradeToNeither = lambdaOpt is not null && CanDowngradeConversionFromLambdaToNeither(BetterResult.Right, lambdaOpt, type1, type2, ref useSiteDiagnostics, true);
                return BetterResult.Right;
            }
            /* TODO:MetaDslx
        ...
        */
        private bool CanDowngradeConversionFromLambdaToNeither(BetterResult currentResult, LambdaExpressionSymbol lambda, TypeSymbol type1, TypeSymbol type2, ref HashSet<DiagnosticInfo> useSiteDiagnostics, bool fromTypeAnalysis)
        {
        ...
        private bool IdenticalParameters(ImmutableArray<ParameterSymbol> p1, ImmutableArray<ParameterSymbol> p2)
        {
            ...
            out ImmutableArray<RefKind> parameterRefKinds)
        {
            ...
            EffectiveParameters effectiveParameters = expanded ?
                GetEffectiveParametersInExpandedForm(method, arguments, argToParamMap, isMethodGroupConversion, allowRefOmittedArguments, out hasAnyRefOmittedArgument) :
                GetEffectiveParametersInNormalForm(method, arguments, argToParamMap, isMethodGroupConversion, allowRefOmittedArguments, out hasAnyRefOmittedArgument);
        ...
        {
            bool discarded;
            return GetEffectiveParametersInNormalForm(member, arguments, argToParamMap, isMethodGroupConversion, allowRefOmittedArguments, hasAnyRefOmittedArgument: out discarded);
        }
            ...
            out bool hasAnyRefOmittedArgument) where TMember : MemberSymbol
        {
            ...
            if (arguments.Arguments.Count == parameterCount && argToParamMap.IsDefaultOrEmpty)
            {
                ImmutableArray<RefKind> parameterRefKinds = GetParameterRefKinds(member);
                if (parameterRefKinds.IsDefaultOrEmpty)
                {
                    return new EffectiveParameters(GetParameterTypes(member), parameterRefKinds);
                }
            }
            ...
            for (int arg = 0; arg < arguments.Arguments.Count; ++arg)
            {
                ...
                RefKind argRefKind = arguments.Arguments[arg].RefKind;
            ...
            ref bool hasAnyRefOmittedArgument)
        {
            ...
            // During argument rewriting, we will replace the argument value with a temporary local and pass that local by reference.
            if (allowRefOmittedArguments && paramRefKind == RefKind.Ref && argRefKind == RefKind.None /*&& !binder.InAttributeArgument*/) // TODO:MetaDslx
            {
        ...
        private EffectiveParameters GetEffectiveParametersInExpandedForm<TMember>(
            ...
            ImmutableArray<int> argToParamMap,
            bool isMethodGroupConversion,
            bool allowRefOmittedArguments) where TMember : MemberSymbol
        {
            bool discarded;
            return GetEffectiveParametersInExpandedForm(member, arguments, argToParamMap, isMethodGroupConversion, allowRefOmittedArguments, hasAnyRefOmittedArgument: out discarded);
        }
            ...
            out bool hasAnyRefOmittedArgument) where TMember : MemberSymbol
        {
            ...
            for (int arg = 0; arg < arguments.Arguments.Count; ++arg)
            {
                ...
                var argRefKind = arguments.Arguments[arg].RefKind;
        ...
        private MemberResolutionResult<TMember> IsMemberApplicableInNormalForm<TMember>(
            ...
            where TMember : MemberSymbol
        {
            ArgumentAnalysisResult argumentAnalysis = ArgumentAnalysisResult.NormalForm(default(ImmutableArray<int>));

            // AnalyzeArguments matches arguments to parameter names and positions. 
            // For that purpose we use the most derived member.
            if (member is MethodLikeSymbol method)
            {
                argumentAnalysis = AnalyzeArguments(method, arguments, isMethodGroupConversion, expanded: false);
            }
            if (!argumentAnalysis.IsValid)
            {
            ...
            where TMember : MemberSymbol
        {
            ...
            if (member is MethodLikeSymbol method)
            {
                argumentAnalysis = AnalyzeArguments(method, arguments, isMethodGroupConversion: false, expanded: true);
            }
            if (!argumentAnalysis.IsValid)
            {
                return new MemberResolutionResult<TMember>(member, leastOverriddenMember, MemberAnalysisResult.ArgumentParameterMismatch(argumentAnalysis));
            ...
            where TMember : MemberSymbol
        {
            ...
            if (member is MethodLikeSymbol method && method.Arity > 0)
            {
                ...
                else
                {
                    ...
                    member = (TMember)method.Construct(typeArguments);
                    leastOverriddenMember = (TMember)leastOverriddenMethod.ConstructedFrom.Construct(typeArguments);
                    ...
                    var parameterTypes = GetParameterTypes(leastOverriddenMember);
        ...
        private ImmutableArray<TypeWithAnnotations> InferMethodTypeArguments(
            ...
            ref HashSet<DiagnosticInfo> useSiteDiagnostics)
        {
            var args = arguments.Arguments.Select(arg => arg.Value).ToImmutableArray();
            ...
            ref HashSet<DiagnosticInfo> useSiteDiagnostics)
        {
            ...
            for (int argumentPosition = 0; argumentPosition < paramCount; argumentPosition++)
            {
                ...
                if (isVararg && argumentPosition == paramCount - 1)
                {
                    ...
                    if (argument.IsArgList)
                    {
                        conversion = Conversions.Identity;
                    }
                    else
                    {
                        badArguments = badArguments ?? ArrayBuilder<int>.GetInstance();
                        badArguments.Add(argumentPosition);
                        conversion = Conversions.NoConversion;
                    }
                }
                else
                {
                    ...
                    if (conversion != Conversions.NoConversion)
                    {
                        badArguments = badArguments ?? ArrayBuilder<int>.GetInstance();
                        badArguments.Add(argumentPosition);
                    }
                ...
                else if (conversion != Conversions.Identity)
                {
                    conversions = ArrayBuilder<Conversion>.GetInstance(paramCount);
                    conversions.AddMany(Conversions.Identity, argumentPosition);
                    conversions.Add(conversion);
                }
            ...
            bool forExtensionMethodThisArg)
        {
            ...
            if (!(argRefKind == parRefKind ||
                 (argRefKind == RefKind.None && argument.Type is DynamicTypeSymbol)))
            {
            ...
            if (ignoreOpenTypes && ContainsTypeParameter(parameterType, parameterContainer: (MethodSymbol)candidate))
            {
                // defer applicability check to runtime:
                return Conversions.ImplicitDynamic;
            }

            var argType = argument.Type;
            if (argRefKind == RefKind.Out &&
               (argument is ReferenceExpressionSymbol refExpr && refExpr.IsDeclaration ||
                argument is DiscardExpressionSymbol discard && discard.Type is null))
            {
            ...
            if (argRefKind == RefKind.None)
            {
                ...
                Debug.Assert((conversion != Conversions.NoConversion) || conversion.IsImplicit, "ClassifyImplicitConversion should only return implicit conversions");
                return conversion;
            }
        ...
        private static TMember GetConstructedFrom<TMember>(TMember member) where TMember : MemberSymbol
        {
            return (TMember)member.ConstructedFrom;
        }

        // TODO:MetaDslx - generalize the following methods
        ...
        private static bool IsValidParams(MemberSymbol member)
        {
            if (member is not MethodLikeSymbol method) return false;
            ...
            ParameterSymbol final = method.Parameters.Last();
        ...
        /// </summary>
        private static int CustomModifierCount(MemberSymbol member)
        {
            // TODO:MetaDslx
            return 0;
        }
        ...
        /// </summary>
        private static int CustomModifierCount(TypeSymbol type)
        {
            // TODO:MetaDslx
            return 0;
        }
