...
namespace MetaDslx.CodeAnalysis.Binding
{
    ...
    public class OverloadResolutionResult<TMember> where TMember : MemberSymbol
    {
            ...
            TypeSymbol delegateOrFunctionPointerType = null) where T : Symbol
        {
            ...
            if (HadAmbiguousWorseMethods(diagnostics, symbols, location, receiver, name))
            {
                return;
            }
            ...
            if (HadStaticInstanceMismatch(diagnostics, symbols, invokedExpression?.GetLocation() ?? location, receiver, nodeOpt, delegateOrFunctionPointerType))
            {
                return;
            }
            ...
            if (HadBadArguments(diagnostics, name, arguments, symbols, location, isMethodGroupConversion))
            {
                return;
            }
            ...
            if (HadConstructedParameterFailedConstraintCheck(binder.Compilation, diagnostics, location))
            {
                return;
            }
            ...
            if (TypeInferenceFailed(binder, diagnostics, symbols, receiver, arguments, location))
            {
                return;
            }
            ...
            if (firstSupported.IsNotNull)
            {
                ...
                {
                    // TODO:MetaDslx
                    //ReportWrongCallingConvention(location, diagnostics, symbols, firstSupported, ((DelegateTypeSymbol)delegateOrFunctionPointerType).Signature);
                    return;
                }
            }
        ...
        private static void ReportUnsupportedMetadata(Location location, DiagnosticBag diagnostics, ImmutableArray<Symbol> symbols, MemberResolutionResult<TMember> firstUnsupported)
        {
            ...
            var messageProvider = diagInfo.MessageProvider as ErrorCodeMessageProvider;
            Debug.Assert(diagInfo != null && messageProvider != null);
            ...
            diagInfo = new DiagnosticInfoWithSymbols(
                messageProvider.GetErrorCode(diagInfo.Code),
                diagInfo.Arguments,
                symbols);
            ...
            Location location)
        {
            ...
            diagnostics.Add(new DiagnosticInfoWithSymbols(
                InternalErrorCode.ERR_BadAccess,
            ...
            TypeSymbol delegateOrFunctionPointerType)
        {
            ...
            var symbol = staticInstanceMismatch.Member;

            ErrorCode errorCode = symbol.IsStatic ? InternalErrorCode.ERR_ObjectRequired : InternalErrorCode.ERR_ObjectProhibited;
        ...
        private bool HadReturnMismatch(Location location, DiagnosticBag diagnostics, RefKind refKind, TypeSymbol delegateOrFunctionPointerType)
        {
            ...
            if (!mismatch.IsNull)
            {
                diagnostics.Add(InternalErrorCode.ERR_DelegateRefMismatch, location, mismatch.Member, delegateOrFunctionPointerType);
                return true;
            }
        ...
        private bool HadConstraintFailure(Location location, DiagnosticBag diagnostics)
        {
            ...
            {
                diagnostics.Add(pair.DiagnosticInfo.ToDiagnostic(location));
            }
            ...
            Location location)
        {
            ...
            if (inferenceFailed.IsNotNull)
            {
                ...
                diagnostics.Add(new DiagnosticInfoWithSymbols(
                    InternalErrorCode.ERR_CantInferMethTypeArgs,
            ...
            if (inferenceFailed.IsNotNull)
            {
                ...
                diagnostics.Add(new DiagnosticInfoWithSymbols(
                    InternalErrorCode.ERR_NoSuchMemberOrExtension,
                    new object[] { instanceArgument.Value.Type, inferenceFailed.Member.Name },
                    symbols), location);
            ...
            ImmutableArray<Symbol> symbols)
        {
            ...
            var badArgument = arguments.Arguments[badArg];
            // We would not have gotten this error had there not been a named argument.
            var badName = badArgument.Name;
            Debug.Assert(!string.IsNullOrEmpty(badName));

            Location location = badArgument.Locations.FirstOrNone();

            // Named argument 'x' specifies a parameter for which a positional argument has already been given
            diagnostics.Add(new DiagnosticInfoWithSymbols(
                InternalErrorCode.ERR_NamedArgumentUsedInPositional,
                new object[] { badName },
                symbols), location);
        }
            ...
            ImmutableArray<Symbol> symbols)
        {
            ...
            diagnostics.Add(new DiagnosticInfoWithSymbols(
                InternalErrorCode.ERR_BadNonTrailingNamedArgument,
                new object[] { badName },
                symbols), location);
        }

        private void ReportDuplicateNamedArgument(MemberResolutionResult<TMember> bad, DiagnosticBag diagnostics, AnalyzedArguments arguments)
        {
            Debug.Assert(bad.Result.BadArgumentsOpt.Length == 1);
            int badArg = bad.Result.BadArgumentsOpt[0];
            var badArgument = arguments.Arguments[badArg];
            // We would not have gotten this error had there not been a named argument.
            var badName = badArgument.Name;
            ...
            diagnostics.Add(InternalErrorCode.ERR_DuplicateNamedArgument.ToDiagnostic(location, badName));
        }
            ...
            ImmutableArray<Symbol> symbols)
        {
            ...
            InternalErrorCode code = (object)delegateTypeBeingInvoked != null ?
                InternalErrorCode.ERR_BadNamedArgumentForDelegateInvoke :
                InternalErrorCode.ERR_BadNamedArgument;
            ...
            Location location)
        {
            ...
            {
                Debug.Assert(badMember is MethodLikeSymbol);
                Debug.Assert(badMember.IsVarArg);
                var varArgParam = badMember.Parameters.FirstOrDefault(p => p.IsVarArg);
                badParamName = varArgParam?.Name;
            }
            else
            {
            ...
            diagnostics.Add(new DiagnosticInfoWithSymbols(
                InternalErrorCode.ERR_NoCorrespondingArgument,
            ...
            NamedTypeSymbol delegateTypeBeingInvoked)
        {
            ...
            (ErrorCode code, object target) = (typeContainingConstructor, delegateTypeBeingInvoked) switch
            {
                (object t, _) => (InternalErrorCode.ERR_BadCtorArgCount, t),
                (_, object t) => (InternalErrorCode.ERR_BadDelArgCount, t),
                _ => (InternalErrorCode.ERR_BadArgCount, name)
            };
            ...
            Location location)
        {
            ...
            //TypeSymbol formalParameterType = method.Parameters[result.Result.BadParameter].Type;
            //formalParameterType.CheckAllConstraints(new ConstraintsHelper.CheckConstraintsArgs(compilation, includeNullability: false, location, diagnostics));
            ...
            bool isMethodGroupConversion)
        {
            ...
            {
                ReportBadArgumentError(diagnostics, name, arguments, symbols, location, badArg, method, arg);
            }
            ...
            int arg)
        {
            ...
            if (method.IsVarArg && parm == method.Parameters.Length)
            {
                ...
                diagnostics.Add(
                    InternalErrorCode.ERR_BadArgType,
                    sourceLocation,
                    symbols,
                    arg + 1,
                    argument.Display,
            ...
            ParameterSymbol parameter = method.Parameters[parm];
            bool isLastParameter = method.Parameters.Length == parm + 1; // This is used to later decide if we need to try to unwrap a params array
            ...
            if (argument.Value.Type is not null && argument.Value is DiscardExpressionSymbol)
            {
            ...
            else if (refArg != refParameter && !(refArg == RefKind.None && refParameter == RefKind.In))
            {
                if (refParameter == RefKind.None || refParameter == RefKind.In)
                {
                    ...
                    diagnostics.Add(
                        InternalErrorCode.ERR_BadArgExtraRef,
                        sourceLocation,
                        symbols,
                        arg + 1,
                ...
                else
                {
                    ...
                    diagnostics.Add(
                        InternalErrorCode.ERR_BadArgRef,
                        sourceLocation,
                        symbols,
                        arg + 1,
            ...
            {
                Debug.Assert(argument.Value is not DiscardExpressionSymbol || argument.Value.Type is not null);
                ...
                if (arguments.IsExtensionMethodThisArgument(arg))
                {
                    ...
                    diagnostics.Add(
                        InternalErrorCode.ERR_BadInstanceArgType,
                        sourceLocation,
                        symbols,
                        argument.Display,
                        name,
        ...
        private static Symbol UnwrapIfParamsArray(ParameterSymbol parameter, bool isLastParameter)
        {
            ...
            if (parameter.IsVarArg && isLastParameter)
            {
        ...
        private bool HadAmbiguousWorseMethods(DiagnosticBag diagnostics, ImmutableArray<Symbol> symbols, Location location, ExpressionSymbol receiver, string name)
        {
        ...
        private static DiagnosticInfoWithSymbols CreateAmbiguousCallDiagnosticInfo(Symbol first, Symbol second, ImmutableArray<Symbol> symbols)
        {
            var arguments = (first.ContainingNamespace != second.ContainingNamespace) ?
                new object[]
                    {
                            first.ToDisplayString(SymbolDisplayFormat.QualifiedNameOnlyFormat),
                            second.ToDisplayString(SymbolDisplayFormat.QualifiedNameOnlyFormat),
                    } :
                new object[]
                    {
                            first,
            ...
            return new DiagnosticInfoWithSymbols(InternalErrorCode.ERR_AmbigCall, arguments, symbols);
        }
