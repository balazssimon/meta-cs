        ...
        private struct Reader
        {
                ...
                out BlendedNode blendedNode)
            {
                ...
                blendedNode = CreateBlendedNode(
                    ...
                    token: (InternalSyntaxToken)currentNodeOrToken.AsToken().Node);
                return true;
            }
            ...
            private bool CanReuse(SyntaxNodeOrToken nodeOrToken)
            {
                ...
                if (IsFabricatedToken(nodeOrToken.GetKind()))
                {
                    return false;
                }
                ...
                if ((nodeOrToken.IsToken && nodeOrToken.AsToken().IsMissing) ||
                    (nodeOrToken.IsNode && IsIncomplete((LanguageSyntaxNode)nodeOrToken.AsNode())))
                {
                    return false;
                }
            ...
            private bool IsFabricatedToken(SyntaxKind kind)
            {
                switch (kind)
                {
                    default:
