...
namespace MetaDslx.CodeAnalysis
{
    ...
    public abstract partial class LanguageSyntaxNode : SyntaxNodeAdapter, IFormattable
    {
        ...
        /// </summary>
        internal protected new LanguageSyntaxNode Parent
        {
            get
            {
        ...
        internal protected new LanguageSyntaxNode ParentOrStructuredTriviaParent
        {
            get
            {
        ...
        internal IList<LanguageSyntaxNode> GetDirectives(Func<LanguageSyntaxNode, bool> filter = null)
        {
            return ((SyntaxNodeOrToken)this).GetDirectives<LanguageSyntaxNode>(filter);
        }
        ...
        public IDirectiveTriviaSyntax GetFirstDirective(Func<IDirectiveTriviaSyntax, bool> predicate = null)
        {
            foreach (var child in this.ChildNodesAndTokens())
            {
                if (child.ContainsDirectives)
                {
                    if (child.IsNode)
                    {
                        var d = ((LanguageSyntaxNode)child.AsNode()).GetFirstDirective(predicate);
                        if (d != null)
                        {
                            return d;
                        }
        ...
        public IDirectiveTriviaSyntax GetLastDirective(Func<IDirectiveTriviaSyntax, bool> predicate = null)
        {
            foreach (var child in this.ChildNodesAndTokens().Reverse())
            {
                if (child.ContainsDirectives)
                {
                    if (child.IsNode)
                    {
                        var d = ((LanguageSyntaxNode)child.AsNode()).GetLastDirective(predicate);
                        if (d != null)
                        {
                            return d;
                        }
        ...
        {
            return Language.SyntaxFactory.AreEquivalent(this, (LanguageSyntaxNode)node, topLevel);
        }
