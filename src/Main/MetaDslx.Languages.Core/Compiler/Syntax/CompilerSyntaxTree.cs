...
namespace MetaDslx.Languages.Compiler
{
    ...
    /// </summary>
    public abstract partial class CompilerSyntaxTree : LanguageSyntaxTree
    {
        internal static readonly CompilerSyntaxTree Dummy = new DummySyntaxTree();
        /// <summary>
        /// The options used by the parser to produce the syntax tree.
        /// </summary>
        public new abstract CompilerParseOptions Options { get; }
        /// <summary>
        /// Gets the root node of the syntax tree.
        /// </summary>
        public new abstract CompilerSyntaxNode GetRoot(CancellationToken cancellationToken = default(CancellationToken));
        /// <summary>
        /// Gets the root node of the syntax tree if it is already available.
        /// </summary>
        public abstract bool TryGetRoot(out CompilerSyntaxNode root);
        /// <summary>
        ...
        /// </remarks>
        public new virtual Task<CompilerSyntaxNode> GetRootAsync(CancellationToken cancellationToken = default(CancellationToken))
        {
            CompilerSyntaxNode node;
        ...
        /// </summary>
        public static CompilerSyntaxTree Create(CompilerSyntaxNode root, ParseData parseData, CompilerParseOptions? options = null, string path = "", SourceText? text = null, Encoding? encoding = null, SourceHashAlgorithm checksumAlgorithm = SourceHashAlgorithm.Sha1)
        {
            if (root == null)
            {
                throw new ArgumentNullException(nameof(root));
            }
            var directives = root.Kind == CompilerSyntaxKind.Main && root is ICompilationUnitRootSyntax compilationUnitRoot ?
            ...
            return new ParsedSyntaxTree(
                ...
                path: path,
                options: options ?? CompilerParseOptions.Default,
                root: root,
        ...
        internal static CompilerSyntaxTree CreateForDebugger(CompilerSyntaxNode root, ParseData parseData, SourceText text, CompilerParseOptions options)
        {
        ...
        /// <summary>
        /// <para>
        /// Internal helper for <see cref="CompilerSyntaxNode"/> class to create a new syntax tree rooted at the given root node.
        /// This method does not create a clone of the given root, but instead preserves it's reference identity.
        /// </para>
        /// <para>NOTE: This method is only intended to be used from <see cref="CompilerSyntaxNode.SyntaxTree"/> property.</para>
        /// <para>NOTE: Do not use this method elsewhere, instead use <see cref="Create(CompilerSyntaxNode, CSharpParseOptions, string, Encoding)"/> method for creating a syntax tree.</para>
        /// </summary>
        internal static CompilerSyntaxTree CreateWithoutClone(CompilerSyntaxNode root, ParseData parseData)
        {
            Debug.Assert(root != null);
            return new ParsedSyntaxTree(
                textOpt: null,
                ...
                path: "",
                options: CompilerParseOptions.Default,
                root: root,
        ...
        /// </summary>
        public static CompilerSyntaxTree ParseText(
            string text,
            ...
            CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            options = options ?? CompilerParseOptions.Default;
            using (var parser = new CompilerSyntaxParser(text, options, oldTree: null, oldParseData: ParseData.Empty, changes: null, cancellationToken: cancellationToken))
            {
        ...
        {
            return new Antlr4CompletionSource(this.ParseData, this.GetRoot(cancellationToken), CompilerParser._ATN).GetTokenSuggestions(position, cancellationToken);
        }
        public new ImmutableArray<CompilerSyntaxKind> LookupTokens(int position, CancellationToken cancellationToken)
        {
            return this.LookupTokensCore(position, cancellationToken).Select(kind => (CompilerSyntaxKind)kind).ToImmutableArray();
        }
        #endregion
        #region Changes
        /// <summary>
        ...
        private SyntaxTree WithChanges(SourceText newText, IReadOnlyList<TextChangeRange> changes)
        {
            ...
            using (var parser = new CompilerSyntaxParser(newText, this.Options, oldTree?.GetRoot(), oldTree?.ParseData ?? ParseData.Empty, changes))
            {
                var compilationUnit = (MainSyntax)parser.Parse();
        ...
        internal sealed class DummySyntaxTree : CompilerSyntaxTree
        {
            ...
            public override CompilerParseOptions Options
            {
            ...
            {
                return CompilerLanguage.Instance.SyntaxFactory.SyntaxTree((LanguageSyntaxNode)root, options: (CompilerParseOptions)options, path: FilePath, encoding: null);
            }
            public override SyntaxTree WithFilePath(string path)
            {
                return CompilerLanguage.Instance.SyntaxFactory.SyntaxTree(_node, options: this.Options, path: path, encoding: null);
            }
        }
        private class ParsedSyntaxTree : CompilerSyntaxTree
        {
            ...
            private readonly CompilerSyntaxNode _root;
            ...
            internal ParsedSyntaxTree(SourceText? textOpt, Encoding? encodingOpt, SourceHashAlgorithm checksumAlgorithm, string path, CompilerParseOptions options, CompilerSyntaxNode root, ParseData parseData, bool cloneRoot = true)
            {
                Debug.Assert(root != null);
        ...
        {
            public DebuggerSyntaxTree(CompilerSyntaxNode root, ParseData parseData, SourceText text, CompilerParseOptions options)
                : base(
                    text,
                    text.Encoding,
