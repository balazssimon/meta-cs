        ...
        protected override void ForceCompleteNode(CancellationToken cancellationToken)
        {
            ...
            switch (syntax.Kind.Switch())
            {
                ...
                case MetaSyntaxKind.FieldDeclaration:
                    ...
                    this.SetDocumentation(mobj, fld.FieldModifier?.GetFirstToken() ?? fld.TypeReference.GetFirstToken(), fld.Attribute);
                    break;
                case MetaSyntaxKind.OperationDeclaration:
                    var op = (OperationDeclarationSyntax)syntax;
                    this.SetDocumentation(mobj, op.OperationModifier.FirstOrDefault()?.GetFirstToken() ?? op.ReturnType.GetFirstToken(), op.Attribute);
                    break;
                default:
                    break;
            }
