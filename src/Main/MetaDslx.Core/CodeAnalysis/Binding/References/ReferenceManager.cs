...
namespace MetaDslx.CodeAnalysis.Binding
{
    ...
    internal sealed class ReferenceManager : CommonReferenceManager<LanguageCompilation, AssemblySymbol>
    {
        ...
        {
            var csReference = compilationReference as Microsoft.CodeAnalysis.CSharp.CSharpCompilationReference;
            var languageReference = compilationReference as LanguageCompilationReference;
            if (csReference == null && languageReference == null)
            {
            ...
            AssemblyData result = null;
            if (csReference != null)
            {
                result = new AssemblyDataForCompilation(csReference.Compilation, csReference.Properties.EmbedInteropTypes);
                Debug.Assert((object)csReference.Compilation.SourceAssembly != null);
            }
            if (languageReference != null)
            {
                result = new AssemblyDataForCompilation(languageReference.Compilation, languageReference.Properties.EmbedInteropTypes);
        ...
        public void CreateSourceAssemblyForCompilation(LanguageCompilation compilation)
        {
            ...
            else
            {
                ...
                var newManager = new ReferenceManager(_language, this.SimpleAssemblyName, this.IdentityComparer, this.ObservedMetadata);
        ...
        private sealed class AssemblyDataForCompilation : AssemblyDataForMetadataOrCompilation
        {
            ...
            private static ImmutableArray<AssemblyIdentity> GetReferencedAssemblies(Compilation compilation)
            {
                ...
                var sourceReferencedAssemblies = modules.First().ReferencedAssemblies;
                var sourceReferencedAssemblySymbols = modules.First().ReferencedAssemblySymbols;
                ...
                {
                    result.AddRange(module.ReferencedAssemblies);
                }
