...
namespace MetaDslx.CodeAnalysis.Syntax
{
    internal static class SyntaxNodeRemover
    {
            ...
            where TRoot : SyntaxNode
        {
            ...
            var remover = new SyntaxRemover(root.Language(), nodes.ToArray(), options);
        ...
        private class SyntaxRemover : SyntaxRewriter
        {
            ...
            public SyntaxRemover(
                ...
                SyntaxRemoveOptions options)
                : base(language, nodesToRemove.Any(n => n.IsPartOfStructuredTrivia()))
            {
            ...
            private static SyntaxTrivia? GetEndOfLine(SyntaxTriviaList list)
            {
                foreach (var trivia in list)
                {
                    ...
                    if (trivia.IsDirective && trivia.GetStructure() is IDirectiveTriviaSyntax directive)
                    {
            ...
            private void AddDirectives(SyntaxNode node, TextSpan span)
            {
                if (node.ContainsDirectives)
                {
                    ...
                    var directivesInSpan = node.DescendantTrivia(span, n => n.ContainsDirectives, descendIntoTrivia: true)
                                         ...
                                         .Select(tr => (IDirectiveTriviaSyntax)tr.GetStructure());

                    foreach (var directive in directivesInSpan)
                    {
                        if ((_options & SyntaxRemoveOptions.KeepDirectives) != 0)
                        {
                            _directivesToKeep.Add((SyntaxNode)directive);
                        }
                        else if (directive.Directive.Kind == DirectiveKind.Define ||
                            directive.Directive.Kind == DirectiveKind.Undef)
                        {
                            // always keep #define and #undef, even if we are only keeping unbalanced directives
                            _directivesToKeep.Add((SyntaxNode)directive);
                        }
                        else if (directive.Directive.HasRelatedDirectives)
                        {
                            // a balanced directive with respect to a given node has all related directives rooted under that node
                            var relatedDirectives = directive.GetRelatedDirectives().Cast<SyntaxNode>();
                        ...
                        if (_directivesToKeep.Contains((SyntaxNode)directive))
                        {
