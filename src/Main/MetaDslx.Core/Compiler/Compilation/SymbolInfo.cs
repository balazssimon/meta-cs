...
namespace MetaDslx.Compiler
{
    ...
    {
        internal static readonly SymbolInfo None = new SymbolInfo(null, ImmutableArray<ISymbol0>.Empty, CandidateReason.None);

        private ImmutableArray<ISymbol0> _candidateSymbols;
        ...
        /// </summary>
        public ImmutableArray<ISymbol0> CandidateSymbols
        {
            get
            {
        ...
        internal ImmutableArray<ISymbol0> GetAllSymbols()
        {
            if (this.Symbol != null)
            {
                return ImmutableArray.Create<ISymbol0>(this.Symbol);
            }
            else
            {
        ...
        internal SymbolInfo(ISymbol0 symbol)
            : this(symbol, ImmutableArray<ISymbol0>.Empty, CandidateReason.None)
        {
        }

        internal SymbolInfo(ISymbol0 symbol, CandidateReason reason)
            : this(symbol, ImmutableArray<ISymbol0>.Empty, reason)
        {
        }

        internal SymbolInfo(ImmutableArray<ISymbol0> candidateSymbols, CandidateReason candidateReason)
            : this(null, candidateSymbols, candidateReason)
        {
        }

        internal SymbolInfo(ISymbol0 symbol, ImmutableArray<ISymbol0> candidateSymbols, CandidateReason candidateReason)
            : this()
        {
            this.Symbol = symbol;
            _candidateSymbols = candidateSymbols.IsDefault ? ImmutableArray.Create<ISymbol0>() : candidateSymbols;

#if DEBUG
            const int NamespaceKindNamespaceGroup = 0;
            Debug.Assert((object)symbol == null || symbol.MIsNamespace || ((INamespaceSymbol)symbol).NamespaceKind != NamespaceKindNamespaceGroup);
            foreach (var item in _candidateSymbols)
            {
                Debug.Assert(!symbol.MIsNamespace || ((INamespaceSymbol)item).NamespaceKind != NamespaceKindNamespaceGroup);
            }
#endif
