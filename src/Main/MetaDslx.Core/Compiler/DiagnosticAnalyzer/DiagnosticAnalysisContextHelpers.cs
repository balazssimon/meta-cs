...
namespace MetaDslx.Compiler.Diagnostics
{
    internal static class DiagnosticAnalysisContextHelpers
    {
        ...
        internal static void VerifyArguments<TContext>(Action<TContext> action, ImmutableArray<ModelSymbolInfo> symbolKinds)
        {
            VerifyAction(action);
        ...
        internal static void VerifyArguments(Diagnostic diagnostic, Compilation compilationOpt, Func<Diagnostic, bool> isSupportedDiagnostic)
        {
            ...
            {
                throw new ArgumentException(string.Format(CompilerResources.UnsupportedDiagnosticReported, diagnostic.Id), nameof(diagnostic));
            }
            ...
            if (!UnicodeCharacterUtilities.IsValidIdentifier(diagnostic.Id))
            {
                ...
                throw new ArgumentException(string.Format(CompilerResources.InvalidDiagnosticIdReported, diagnostic.Id), nameof(diagnostic));
            }
        }
        ...
        private static void VerifyDiagnosticLocationInCompilation(string id, Location location, Compilation compilation)
        {
            ...
            if (!compilation.ContainsSyntaxTree(location.SourceTree))
            {
                ...
                throw new ArgumentException(string.Format(CompilerResources.InvalidDiagnosticLocationReported, id, location.SourceTree.FilePath), "diagnostic");
            }

            if (location.SourceSpan.End > location.SourceTree.Length)
            {
                // Disallow diagnostics with source locations outside this compilation.
                throw new ArgumentException(string.Format(CompilerResources.InvalidDiagnosticSpanReported, id, location.SourceSpan, location.SourceTree.FilePath), "diagnostic");
            }
        }
        ...
        private static void VerifyAction<TContext>(Action<TContext> action)
        {
            ...
            {
                throw new ArgumentException(CompilerResources.AsyncAnalyzerActionCannotBeRegistered, nameof(action));
            }
        }

        private static void VerifySymbolKinds(ImmutableArray<ModelSymbolInfo> symbolKinds)
        {
            ...
            {
                throw new ArgumentException(CompilerResources.ArgumentCannotBeEmpty, nameof(symbolKinds));
            }
        }
            ...
            where TLanguageKindEnum : struct
        {
            ...
            {
                throw new ArgumentException(CompilerResources.ArgumentCannotBeEmpty, nameof(syntaxKinds));
            }
        }
        ...
        private static void VerifyOperationKinds(ImmutableArray<OperationKind> operationKinds)
        {
            ...
            {
                throw new ArgumentException(CompilerResources.ArgumentCannotBeEmpty, nameof(operationKinds));
            }
        }
