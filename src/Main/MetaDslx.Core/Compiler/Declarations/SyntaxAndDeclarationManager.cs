...
namespace MetaDslx.Compiler.Declarations
{
    internal sealed partial class SyntaxAndDeclarationManager : CommonSyntaxAndDeclarationManager
    {
            ...
            ref DeclarationTable declTable)
        {
            ...
            foreach (var directive in tree.GetRoot().GetLoadDirectives())
            {
                ...
                else
                {
                    ...
                    else if (!loadedSyntaxTreeMapBuilder.ContainsKey(resolvedFilePath))
                    {
                        ...
                        {
                            diagnostics.Add(
                                CommonCompiler.ToFileReadDiagnostics(compilation, e, resolvedFilePath),
                                fileToken.GetLocation());
                        }
                    }
                    else
        ...
        public SyntaxAndDeclarationManager ReplaceSyntaxTree(SyntaxTree oldTree, SyntaxTree newTree)
        {
            ...
            var newLoadDirectivesSyntax = ((ICompilationUnitSyntax)newTree.GetRoot()).GetLoadDirectives();
            var loadDirectivesHaveChanged = !((ICompilationUnitSyntax)oldTree.GetRoot()).GetLoadDirectives().SequenceEqual(newLoadDirectivesSyntax);
            var syntaxTrees = state.SyntaxTrees;
