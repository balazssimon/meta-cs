...
namespace MetaDslx.Compiler
{
    ...
    public abstract partial class SyntaxRewriter : SyntaxVisitor<SyntaxNode, SyntaxNode>
    {
        ...
        public virtual SyntaxTrivia VisitTrivia(SyntaxTrivia trivia)
        {
            ...
            {
                var structure = (SyntaxNode)trivia.GetStructure();
                var newStructure = this.Visit(structure);
                if (newStructure != structure)
                {
                    if (newStructure != null)
        ...
        {
            Syntax.SyntaxListBuilder alternate = null;
            for (int i = 0, n = list.Count; i < n; i++)
            {
                ...
                {
                    alternate = new Syntax.SyntaxListBuilder(n);
        ...
        public virtual SeparatedSyntaxList<TNode> VisitList<TNode>(SeparatedSyntaxList<TNode> list) where TNode : SyntaxNode
        {
            ...
            Syntax.SeparatedSyntaxListBuilder<TNode> alternate = default;
            ...
            for (; i < sepCount; i++)
            {
                ...
                if (alternate.IsNull)
                {
                    ...
                    {
                        alternate = new Syntax.SeparatedSyntaxListBuilder<TNode>(count);
                ...
                if (!alternate.IsNull)
                {
                    if (visitedNode != null)
                    {
                        ...
                        {
                            throw new InvalidOperationException(CompilerResources.SeparatorIsExpected);
                        }
                    ...
                    else
                    {
                        if (visitedNode == null)
                        {
                            throw new InvalidOperationException(CompilerResources.ElementIsExpected);
                        }
                    }
                }
            }
