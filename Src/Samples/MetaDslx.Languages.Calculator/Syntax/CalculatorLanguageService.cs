...
namespace MetaDslx.Languages.Calculator.VisualStudio
{
    public class CalculatorLanguageAuthoringScope : AuthoringScope
    {
	...
	public class CalculatorLanguageColorizer : Colorizer
    {
        public CalculatorLanguageColorizer(LanguageService svc, IVsTextLines buffer, IScanner scanner)
        ...
        public override int GetStateAtEndOfLine(int iLine, int iLength, IntPtr pText, int iState)
        {
            ...
            {
                this.Scanner.SetSource(text, 0);
                ((CalculatorLanguageScanner)this.Scanner).ScanLine(ref iState);
            }
            return iState;
        }
    ...
    public abstract class CalculatorLanguageConfigBase
    {
        private static CalculatorLanguageConfig instance = null;
        public static CalculatorLanguageConfig Instance
        {
            get 
            {
                if (instance == null)
                {
					// If there is a compile error in the following line, create a class CalculatorLanguageConfig
					// which is a subclass of CalculatorLanguageConfigBase
                    instance = new CalculatorLanguageConfig();
                }
                return instance;
            }
        }
        private List<CalculatorLanguageColorableItem> colorableItems = new List<CalculatorLanguageColorableItem>();
        public IList<CalculatorLanguageColorableItem> ColorableItems
        {
        ...
        {
            colorableItems.Add(new CalculatorLanguageColorableItem(name, type, (COLORINDEX)(-1), COLORINDEX.CI_USERTEXT_BK, foregroundColor, Color.White, bold, strikethrough));
        ...
        {
            colorableItems.Add(new CalculatorLanguageColorableItem(name, type, foregroundIndex, COLORINDEX.CI_USERTEXT_BK, Color.Black, Color.White, bold, strikethrough));
            return (TokenColor)colorableItems.Count;
        }
        ...
        {
            colorableItems.Add(new CalculatorLanguageColorableItem(name, type, foregroundIndex, backgroundIndex, foregroundColor, backgroundColor, bold, strikethrough));
            return (TokenColor)colorableItems.Count;
        }
    }
    public class CalculatorLanguageColorableItem : IVsColorableItem, IVsHiColorItem
    {
        ...
        public CalculatorLanguageColorableItem(string displayName, TokenType tokenType, COLORINDEX foregroundIndex, COLORINDEX backgroundIndex, Color foregroundColor, Color backgroundColor, bool bold, bool strikethrough)
        {
    ...
    [ComVisible(true)]
    [Guid(CalculatorLanguageConfig.CalculatorGeneratorServiceGuid)]
    [ProvideObject(typeof(CalculatorGeneratorService), RegisterUsing = RegistrationMethod.CodeBase)]
    [CodeGeneratorRegistration(typeof(CalculatorGeneratorService), CalculatorLanguageConfig.GeneratorName, "{fae04ec1-301f-11d3-bf4b-00c04f79efbc}", GeneratorRegKeyName = CalculatorLanguageConfig.FileExtension)]
    [CodeGeneratorRegistration(typeof(CalculatorGeneratorService), CalculatorLanguageConfig.GeneratorServiceName, "{fae04ec1-301f-11d3-bf4b-00c04f79efbc}", GeneratorRegKeyName = CalculatorLanguageConfig.GeneratorName, GeneratesDesignTimeSource = true)]
    public class CalculatorGeneratorService : VsSingleFileGenerator
    {
		...
		{
			// If there is a compile error in the following line, create a class CalculatorGenerator
			// which is a subclass of SingleFileGenerator
			return new CalculatorGenerator(inputFilePath, inputFileContents, defaultNamespace);
		}

        public override string GetDefaultFileExtension()
        {
            return CalculatorGenerator.DefaultExtension;
        }
    }
    public class CalculatorLanguageScanner : IScanner
    {
        private int currentOffset;
        private string currentLine;
        private MetaDslx.Languages.Calculator.Syntax.InternalSyntax.CalculatorLexer lexer;
        private MetaDslx.Languages.Calculator.Syntax.CalculatorSyntaxFacts syntaxFacts;
        private Dictionary<LanguageScannerState, int> inverseStates;
        private Dictionary<int, LanguageScannerState> states;
        private int lastState;
        public CalculatorLanguageScanner()
        {
            this.inverseStates = new Dictionary<LanguageScannerState, int>();
            this.states = new Dictionary<int, LanguageScannerState>();
            this.lastState = 0;
            this.syntaxFacts = MetaDslx.Languages.Calculator.Syntax.CalculatorSyntaxFacts.Instance;
        }
        private void LoadState(int state, MetaDslx.Languages.Calculator.Syntax.InternalSyntax.CalculatorLexer lexer)
        {
            LanguageScannerState value = default(LanguageScannerState);
            this.states.TryGetValue(state, out value);
            value.Restore(lexer);
        }
        private int SaveState(MetaDslx.Languages.Calculator.Syntax.InternalSyntax.CalculatorLexer lexer)
        {
            int result = 0;
        ...
        public bool ScanTokenAndProvideInfoAboutIt(TokenInfo tokenInfo, ref int state)
        {
            try
            {
                if (this.lexer == null)
                {
                    this.lexer = new MetaDslx.Languages.Calculator.Syntax.InternalSyntax.CalculatorLexer(new AntlrInputStream(this.currentLine + "\r\n"));
                }
                ...
                if (tokenType >= 0 && tokenType < CalculatorLanguageConfig.Instance.ColorableItems.Count)
                {
                    CalculatorLanguageColorableItem colorItem = CalculatorLanguageConfig.Instance.ColorableItems[tokenType];
        ...
        {
            public LanguageScannerState(MetaDslx.Languages.Calculator.Syntax.InternalSyntax.CalculatorLexer lexer)
            {
            ...
            public void Restore(MetaDslx.Languages.Calculator.Syntax.InternalSyntax.CalculatorLexer lexer)
            {
    ...
    [ComVisible(true)]
    [Guid(CalculatorLanguageConfig.CalculatorLanguageServiceGuid)]
    public class CalculatorLanguageService : LanguageService
    {
        private LanguagePreferences preferences;
        private CalculatorLanguageScanner scanner;
        public CalculatorLanguageService()
        {
			// Creating the config instance
			CalculatorLanguageConfigBase.Instance.ToString();
        }
        public override string GetFormatFilterList()
        {
            return CalculatorLanguageConfig.FilterList;
        }
        public override LanguagePreferences GetLanguagePreferences()
        {
            if (preferences == null)
            {
                preferences = new LanguagePreferences(this.Site, typeof(CalculatorLanguageService).GUID, this.Name);
                preferences.Init();
            }
            return preferences;
        }
        public override IScanner GetScanner(IVsTextLines buffer)
        {
            if (scanner == null)
            {
                scanner = new CalculatorLanguageScanner();
            }
            return scanner;
        }
        public override Microsoft.VisualStudio.Package.Source CreateSource(IVsTextLines buffer)
        {
            return new CalculatorLanguageSource(this, buffer, this.GetColorizer(buffer));
        }
        #region Custom Colors
        public override int GetColorableItem(int index, out IVsColorableItem item)
        {
            if (index >= 0 && index < CalculatorLanguageConfig.Instance.ColorableItems.Count)
            {
                item = CalculatorLanguageConfig.Instance.ColorableItems[index];
                return Microsoft.VisualStudio.VSConstants.S_OK;
            }
            else
            {
                item = CalculatorLanguageConfig.Instance.ColorableItems[0];
                return Microsoft.VisualStudio.VSConstants.S_OK;
            }
        }
        public override int GetItemCount(out int count)
        {
            count = CalculatorLanguageConfig.Instance.ColorableItems.Count;
            return Microsoft.VisualStudio.VSConstants.S_OK;
        }
        #endregion
        public override void OnIdle(bool periodic)
        {
            ...
            CalculatorLanguageSource src = (CalculatorLanguageSource)GetSource(this.LastActiveTextView);
        ...
        {
            get { return CalculatorLanguageConfig.LanguageName; }
        }
        public override ViewFilter CreateViewFilter(CodeWindowManager mgr, IVsTextView newView)
        {
            return new CalculatorLanguageViewFilter(mgr, newView);
        }
        public override Colorizer GetColorizer(IVsTextLines buffer)
        {
            return new CalculatorLanguageColorizer(this, buffer, this.GetScanner(buffer));
        }
        public override AuthoringScope ParseSource(ParseRequest req)
        {
			try
			{
				CalculatorLanguageSource source = (CalculatorLanguageSource)this.GetSource(req.FileName);
				switch (req.Reason)
				{
					case ParseReason.Check:
						...
						var compilation = new MetaDslx.Languages.Calculator.Compilation.CalculatorCompiler(req.Text, "", inputDir, null, req.FileName);
                        compilation.Compile();
            ...
            return new CalculatorLanguageAuthoringScope();
        }
    }
	public class CalculatorLanguageSource : Microsoft.VisualStudio.Package.Source
    {
        public CalculatorLanguageSource(LanguageService service, IVsTextLines textLines, Colorizer colorizer)
            : base(service, textLines, colorizer)
        {
        }
    }
    public class CalculatorLanguageViewFilter : ViewFilter
    {
        public CalculatorLanguageViewFilter(CodeWindowManager mgr, IVsTextView view)
            : base(mgr, view)
        {
        }
