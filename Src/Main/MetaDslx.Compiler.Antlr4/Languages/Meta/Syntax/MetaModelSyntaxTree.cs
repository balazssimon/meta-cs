...
{
    /// <summary>
    /// The parsed representation of a MetaModel source document.
    /// </summary>
    public abstract class MetaModelSyntaxTree : SyntaxTree
    {
        ...
        private SyntaxTree WithChanges(SourceText newText, IReadOnlyList<TextChangeRange> changes)
        {
            ...
            using (var parser = MetaModelLanguage.Instance.SyntaxFactory.MakeParser(newText, this.Options, oldTree?.GetRoot(), changes))
            {
        ...
        public abstract MetaModelSyntaxTree WithRootAndOptions(SyntaxNode root, ParseOptions options);
        public abstract MetaModelSyntaxTree WithFilePath(string path);
        #region Factories
        /// <summary>
        ...
        internal static MetaModelSyntaxTree CreateForDebugger(MetaModelSyntaxNode root, SourceText text)
        {
        ...
        /// <para>NOTE: Do not use this method elsewhere, instead use <see cref="Create(MetaModelSyntaxNode, MetaModelParseOptions, string, Encoding)"/> method for creating a syntax tree.</para>
        /// </summary>
            ...
            CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            options = options ?? MetaModelParseOptions.Default;
            using (var parser = MetaModelLanguage.Instance.SyntaxFactory.MakeParser(text, options, oldTree: null, changes: null))
            {
                var compilationUnit = (MainSyntax)parser.Parse().CreateRed();
        ...
        {
            public DebuggerSyntaxTree(MetaModelSyntaxNode root, SourceText text)
                : base(
                    text,
                    text.Encoding,
        ...
        private sealed class DummySyntaxTree : MetaModelSyntaxTree
        {
            private readonly MainSyntax _node;
            public DummySyntaxTree()
            {
                _node = new MainSyntax(MetaModelLanguage.Instance.InternalSyntaxFactory.Main(null, true), this, 0);
            }
            ...
            public override MetaModelSyntaxTree WithRootAndOptions(SyntaxNode root, ParseOptions options)
            {
                return MetaModelLanguage.Instance.SyntaxFactory.SyntaxTree(root, options: (MetaModelParseOptions)options, path: FilePath, encoding: null);
            }
            public override MetaModelSyntaxTree WithFilePath(string path)
            {
        ...
        private class ParsedSyntaxTree : MetaModelSyntaxTree
        {
            ...
            internal ParsedSyntaxTree(SourceText textOpt, Encoding encodingOpt, SourceHashAlgorithm checksumAlgorithm, string path, MetaModelParseOptions options, MetaModelSyntaxNode root, DirectiveStack directives, bool cloneRoot = true)
            {
                ...
                _hasCompilationUnitRoot = root.Kind == MetaModelSyntaxKind.Main;
