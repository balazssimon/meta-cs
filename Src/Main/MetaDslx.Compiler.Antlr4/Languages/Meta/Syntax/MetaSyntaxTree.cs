...
namespace MetaDslx.Languages.Meta.Syntax
{
    ...
    /// </summary>
    public abstract class MetaSyntaxTree : SyntaxTree
    {
        ...
        private SyntaxTree WithChanges(SourceText newText, IReadOnlyList<TextChangeRange> changes)
        {
            ...
            using (var parser = MetaLanguage.Instance.SyntaxFactory.MakeParser(newText, this.Options, oldTree?.GetRoot(), changes))
            {
        ...
        public abstract MetaSyntaxTree WithRootAndOptions(SyntaxNode root, ParseOptions options);
        public abstract MetaSyntaxTree WithFilePath(string path);
        #region Factories
        /// <summary>
        ...
        internal static MetaSyntaxTree CreateForDebugger(MetaSyntaxNode root, SourceText text)
        {
        ...
        /// <para>NOTE: Do not use this method elsewhere, instead use <see cref="Create(MetaSyntaxNode, MetaParseOptions, string, Encoding)"/> method for creating a syntax tree.</para>
        /// </summary>
            ...
            CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            options = options ?? MetaParseOptions.Default;
            using (var parser = MetaLanguage.Instance.SyntaxFactory.MakeParser(text, options, oldTree: null, changes: null))
            {
                var compilationUnit = (MainSyntax)parser.Parse().CreateRed();
        ...
        {
            public DebuggerSyntaxTree(MetaSyntaxNode root, SourceText text)
                : base(
                    text,
                    text.Encoding,
        ...
        private sealed class DummySyntaxTree : MetaSyntaxTree
        {
            ...
            public override MetaSyntaxTree WithRootAndOptions(SyntaxNode root, ParseOptions options)
            {
                return MetaLanguage.Instance.SyntaxFactory.SyntaxTree(root, options: (MetaParseOptions)options, path: FilePath, encoding: null);
            }
            public override MetaSyntaxTree WithFilePath(string path)
            {
