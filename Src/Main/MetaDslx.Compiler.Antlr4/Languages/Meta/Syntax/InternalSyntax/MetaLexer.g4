...
LSingleLineComment : '//' ~[\r\n]* -> channel(LComment);
LCommentStart : '/*' -> more, mode(LMultilineComment), channel(LComment);

                    
mode LMultilineComment;

LComment_CrLf : '\r'? '\n' -> more, channel(LComment);
LComment_LineBreak : [\u0085\u2028\u2029] -> more, channel(LComment);
LComment_Text : ~[\*\r\n\u0085\u2028\u2029]+ -> more, channel(LComment);
                    
LComment : '*/' -> mode(DEFAULT_MODE), channel(LComment);
LComment_Star : '*' -> more, channel(LComment);
