...
namespace MetaDslx.Languages.Meta.Binding
{
    public class MetaBinderFactoryVisitor : BinderFactoryVisitor, IMetaSyntaxVisitor<Binder>
    {
        ...
        protected virtual Binder CreateScopeBinder(Binder parentBinder, RedNode node)
        {
            return this.CreateScopeBinderCore(parentBinder, node);
        }

        protected virtual Binder CreateScopeBinderCore(Binder parentBinder, RedNode node)
        {
		...
		public Binder VisitNamespaceBody(NamespaceBodySyntax node)
		{
			...
			{
				resultBinder = this.GetParentBinder(node);
				resultBinder = this.CreateScopeBinder(resultBinder, node);
				this.BinderFactory.TryAddBinder(node, null, ref resultBinder);
			}
			return resultBinder;
		}
		...
		public Binder VisitFieldModifier(FieldModifierSyntax node)
		{
			...
			if (!this.BinderFactory.TryGetBinder(node, use, out resultBinder))
			{
				...
				if (use == UseFieldModifier)
				{
					switch (((MetaSyntaxToken)node.FieldModifier).Kind)
					{
		...
		public Binder VisitObjectType(ObjectTypeSyntax node)
		{
			...
			if (!this.BinderFactory.TryGetBinder(node, use, out resultBinder))
			{
				...
				{
					switch (((MetaSyntaxToken)node.ObjectType).Kind)
					{
		...
		public Binder VisitPrimitiveType(PrimitiveTypeSyntax node)
		{
			...
			if (!this.BinderFactory.TryGetBinder(node, use, out resultBinder))
			{
				...
				{
					switch (((MetaSyntaxToken)node.PrimitiveType).Kind)
					{
		...
		public Binder VisitCollectionKind(CollectionKindSyntax node)
		{
			...
			if (!this.BinderFactory.TryGetBinder(node, use, out resultBinder))
			{
				...
				if (use == UseCollectionKind)
				{
					switch (((MetaSyntaxToken)node.CollectionKind).Kind)
					{
