...
{
	public class MetaModelDeclarationTreeBuilderVisitor : DeclarationTreeBuilderVisitor, IMetaModelSyntaxVisitor
	{
        protected MetaModelDeclarationTreeBuilderVisitor(MetaModelSyntaxTree syntaxTree, string scriptClassName, bool isSubmission)
            : base(syntaxTree, scriptClassName, isSubmission)
        {
        }

        public static RootSingleDeclaration ForTree(
            MetaModelSyntaxTree syntaxTree,
		...
		public virtual void VisitClassDeclaration(ClassDeclarationSyntax node)
		{
			...
			try
			{
				...
				try
				{
					...
					{
						switch (((MetaModelSyntaxToken)node.KAbstract).Kind)
						{
							default:
								break;
						}
