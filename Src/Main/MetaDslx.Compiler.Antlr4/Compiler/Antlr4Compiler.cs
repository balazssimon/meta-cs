...
using MetaDslx.Compiler.Core.MetaModel;
...
namespace MetaDslx.Compiler.Antlr4Roslyn
{
    public class Antlr4RoslynCompiler : IAntlrErrorListener<int>, IAntlrErrorListener<IToken>
    {
        ...
        public Antlr4RoslynCompiler(string source, string defaultNamespace, string outputDirectory, string fileName)
        {
            ...
            else if (languageName.EndsWith("Lexer")) languageName = languageName.Substring(0, languageName.Length - 6);
        ...
        public void Generate()
        {
            ...
            this.GeneratedFixedTokensSource = this.GenerateFixedTokensSource();
        ...
        /*
        private void CollectFixedTokens()
        {
            bool added = true;
            while (added)
            {
                added = false;
                foreach (var literalCandidate in this.Grammar.FixedTokenCandidates)
                {
                    Antlr4LexerRule literal = this.Grammar.FixedTokens.FirstOrDefault(l => literalCandidate.FixedToken == l.Name);
                    if (literal != null)
                    {
                        this.Grammar.FixedTokens.Add(literalCandidate);
                        added = true;
                    }
                }
                this.Grammar.FixedTokenCandidates.RemoveAll(l => this.Grammar.FixedTokens.Contains(l));
            }
            foreach (var rule in this.Grammar.ParserRules)
            {
                if (rule.Alternatives.Count > 0)
                {
                    foreach (var alt in rule.Alternatives)
                    {
                        ...
                        {
                            elem.IsFixedToken = this.Grammar.FixedTokens.Any(l => l.Name == elem.Type);
                            if (elem.IsBlock)
                            {
                                foreach (var blockAlt in elem.Alternatives)
                                {
                                    foreach (var blockElem in blockAlt.Elements)
                                    {
                                        blockElem.IsFixedToken = this.Grammar.FixedTokens.Any(l => l.Name == blockElem.Type);
                                    }
                                }
                            }
                        }
        ...
        */
        ...
        private string GenerateFixedTokensSource()
        {
            StringBuilder sb = new StringBuilder();
            foreach (var literal in this.Grammar.FixedTokens)
            {
                sb.AppendFormat("{0}={1}", literal.Name, literal.FixedToken);
                sb.AppendLine();
            }
            return sb.ToString();
        }
        ...
        {
            if (token == null) return new LinePositionSpan();
        ...
        public void SyntaxError(IRecognizer recognizer, int offendingSymbol, int line, int charPositionInLine, string msg, RecognitionException e)
        {
            ...
            if (token != null)
            {
                this.DiagnosticBag.Add(Diagnostic.Create(new DiagnosticDescriptor("AR0001", "Syntax error", msg, "Antlr4.Roslyn", DiagnosticSeverity.Error, true), Location.Create(this.FileName, this.GetTextSpan(token), this.GetLinePositionSpan(token))));
            }
            else
            {
                this.DiagnosticBag.Add(Diagnostic.Create(new DiagnosticDescriptor("AR0001", "Syntax error", msg, "Antlr4.Roslyn", DiagnosticSeverity.Error, true), Location.Create(this.FileName, this.GetTextSpan(recognizer.InputStream.Index), this.GetLinePositionSpan(line, charPositionInLine))));
            }
        }
        ...
        public void SyntaxError(IRecognizer recognizer, IToken offendingSymbol, int line, int charPositionInLine, string msg, RecognitionException e)
        {
            ...
            {
                this.DiagnosticBag.Add(Diagnostic.Create(new DiagnosticDescriptor("AR0001", "Syntax error", msg, "Antlr4.Roslyn", DiagnosticSeverity.Error, true), Location.Create(this.FileName, this.GetTextSpan(offendingSymbol), this.GetLinePositionSpan(offendingSymbol))));
            }
            else
            {
                this.DiagnosticBag.Add(Diagnostic.Create(new DiagnosticDescriptor("AR0001", "Syntax error", msg, "Antlr4.Roslyn", DiagnosticSeverity.Error, true), Location.Create(this.FileName, this.GetTextSpan(recognizer.InputStream.Index), this.GetLinePositionSpan(line, charPositionInLine))));
        ...
        internal void AddError(ITerminalNode token, string message)
        {
            this.DiagnosticBag.Add(Diagnostic.Create(new DiagnosticDescriptor("AR0002", "Syntax error", message, "Antlr4.Roslyn", DiagnosticSeverity.Error, true), Location.Create(this.FileName, this.GetTextSpan(token.Symbol), this.GetLinePositionSpan(token.Symbol))));
        }

        internal void AddWarning(ITerminalNode token, string message)
        {
            this.DiagnosticBag.Add(Diagnostic.Create(new DiagnosticDescriptor("AR0002", "Syntax error", message, "Antlr4.Roslyn", DiagnosticSeverity.Warning, true), Location.Create(this.FileName, this.GetTextSpan(token.Symbol), this.GetLinePositionSpan(token.Symbol))));
        }

        internal void AddError(ParserRuleContext rule, string message)
        {
            this.DiagnosticBag.Add(Diagnostic.Create(new DiagnosticDescriptor("AR0002", "Syntax error", message, "Antlr4.Roslyn", DiagnosticSeverity.Error, true), Location.Create(this.FileName, this.GetTextSpan(rule), this.GetLinePositionSpan(rule))));
        }

        internal void AddWarning(ParserRuleContext rule, string message)
        {
            this.DiagnosticBag.Add(Diagnostic.Create(new DiagnosticDescriptor("AR0002", "Syntax error", message, "Antlr4.Roslyn", DiagnosticSeverity.Warning, true), Location.Create(this.FileName, this.GetTextSpan(rule), this.GetLinePositionSpan(rule))));
        }
    ...
    internal class RoslynRuleCollector : Antlr4RoslynParserBaseVisitor<object>
    {
        ...
        public override object VisitOption(Antlr4RoslynParser.OptionContext context)
        {
            ...
            if (context.identifier().GetText() == "tokenVocab")
            {
                ...
                string literalVocabFileName = Path.Combine(this.compiler.OutputDirectory, tokenVocabName + ".fixedtokens");
        ...
        public override object VisitRuleBlock([NotNull] Antlr4RoslynParser.RuleBlockContext context)
        {
            ...
            if (ruleSpec != null)
            {
                ...
                if (!reportedError && !handled)
                {
                    ...
                    else if (possiblyGood)
                    {
                        handled = true;
                        reportedError = true;
                        this.compiler.AddError(ruleSpec.RULE_REF(), "Rule '" + ruleName + "' cannot be mapped to Roslyn. Give labels to the alternatives.");
                    }
                }
                if (!reportedError && !handled)
                {
                    this.compiler.AddError(ruleSpec.RULE_REF(), "Rule '" + ruleName + "' cannot be mapped to Roslyn. Consider simplifying it.");
                }
            ...
            {
                this.compiler.AddError(context, "Unhandled block.");
            }
        ...
        private bool CheckUniqueElements(Antlr4ParserRule rule, ref bool reportedErrors)
        {
            ...
            for (int i = 0; i < elements.Count; i++)
            {
                ...
                if (renameNeeded)
                {
                    result = false;
                    reportedErrors = true;
                    if (elements[i].Node is ParserRuleContext)
                    {
                        this.compiler.AddError((ParserRuleContext)elements[i].Node, "Element '"+ elements[i].Type+ "' must have a name. Elements with the same type in a rule must have different names.");
                    }
                    else
                    {
                        this.compiler.AddError((ITerminalNode)elements[i].Node, "Element '" + elements[i].Type + "' must have a name. Elements with the same type in a rule must have different names.");
                    }
                }
            }
            return result;
        ...
        private int IsRoslynRuleElement(Antlr4RoslynParser.ElementContext first, Antlr4RoslynParser.ElementContext second, Antlr4RoslynParser.ElementContext third, bool allowBlocks, ref bool reportedErrors, out Antlr4ParserRuleElement element)
        {
            ...
            else if (elem.labeledElement() != null && elem.labeledElement().block() != null)
            {
                ...
                this.compiler.AddError(elem.labeledElement().identifier(), "This block cannot be mapped to Roslyn. Create a separate rule from the block.");
                return 0;
            }
            else if (elem.ebnf() != null)
            {
                if (this.IsRoslynBlockSingleAlt(elem.ebnf().block(), ref reportedErrors, out element))
                {
                    ...
                    if (blockSuffix != null)
                    {
                        ...
                        this.compiler.AddError(blockSuffix, "A block cannot have a '*' or '+' suffix. Create a separate rule from the block.");
                        return 0;
                    }
                    else
                    {
                ...
                this.compiler.AddError(elem.ebnf().block(), "This block cannot be mapped to Roslyn. Create a separate rule from the block.");
                return 0;
            }
            return 0;
        }
        ...
        private bool IsRoslynBlockSingleAlt(Antlr4RoslynParser.BlockContext context, ref bool reportedErrors, out Antlr4ParserRuleElement block)
        {
            if (context.altList().alternative().Length == 1)
            {
                ...
                for (int i = 0; i < elems.Length; ++i)
                {
                    ...
                    {
                        this.compiler.AddError(elems[i], "Cannot map this element in the block to a Roslyn rule. Create a separate rule from this element or from this block.");
                        reportedErrors = true;
                        block = null;
                        return false;
                    }
