...
namespace MetaDslx.Compiler.Text
{
    internal static class EncodedStringText
    {
        ...
        private static byte[] TryGetByteArrayFromStream(Stream data)
        {
            ...
            if (fileStream != null && TryGetByteArrayFromFileStream(data, out buffer))
            {
                return buffer;
            }
        ...
        private static bool TryGetByteArrayFromFileStream(Stream stream, out byte[] buffer)
        {
            ...
            if (length == 0)
            {
                buffer = EmptyCollections.ByteArray;
                return true;
            }
            ...
            return StreamUtilities.TryReadAll(stream, buffer, 0, length) == length;
        }
    }
}
