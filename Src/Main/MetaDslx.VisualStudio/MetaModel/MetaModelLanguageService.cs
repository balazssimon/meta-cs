...
namespace MetaDslx.VisualStudio
{
    ...
    public class MetaModelLanguageScanner : IScanner
    {
        ...
        private MetaDslx.Languages.Meta.Syntax.InternalSyntax.MetaModelLexer lexer;
        private MetaDslx.Languages.Meta.Syntax.MetaModelSyntaxFacts syntaxFacts;
        ...
        public MetaModelLanguageScanner()
        {
            ...
            this.lastState = 0;
            this.syntaxFacts = MetaDslx.Languages.Meta.Syntax.MetaModelSyntaxFacts.Instance;
        }
        private void LoadState(int state, MetaDslx.Languages.Meta.Syntax.InternalSyntax.MetaModelLexer lexer)
        {
            LanguageScannerState value = default(LanguageScannerState);
            this.states.TryGetValue(state, out value);
            value.Restore(lexer);
        }
        private int SaveState(MetaDslx.Languages.Meta.Syntax.InternalSyntax.MetaModelLexer lexer)
        {
            int result = 0;
        ...
        public bool ScanTokenAndProvideInfoAboutIt(TokenInfo tokenInfo, ref int state)
        {
            try
            {
                if (this.lexer == null)
                {
                    this.lexer = new MetaDslx.Languages.Meta.Syntax.InternalSyntax.MetaModelLexer(new AntlrInputStream(this.currentLine + "\r\n"));
                }
        ...
        {
            public LanguageScannerState(MetaDslx.Languages.Meta.Syntax.InternalSyntax.MetaModelLexer lexer)
            {
            ...
            public void Restore(MetaDslx.Languages.Meta.Syntax.InternalSyntax.MetaModelLexer lexer)
            {
    ...
    public class MetaModelLanguageService : LanguageService
    {
        ...
        {
            if (index >= 0 && index < MetaModelLanguageConfig.Instance.ColorableItems.Count)
            {
                item = MetaModelLanguageConfig.Instance.ColorableItems[index];
                return Microsoft.VisualStudio.VSConstants.S_OK;
            }
            else
            {
