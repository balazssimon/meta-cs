...
{
    public class MetaLanguageAuthoringScope : AuthoringScope
    {
	...
	public class MetaLanguageColorizer : Colorizer
    {
        public MetaLanguageColorizer(LanguageService svc, IVsTextLines buffer, IScanner scanner)
        ...
        public override int GetStateAtEndOfLine(int iLine, int iLength, IntPtr pText, int iState)
        {
            ...
            {
                this.Scanner.SetSource(text, 0);
                ((MetaLanguageScanner)this.Scanner).ScanLine(ref iState);
            }
            return iState;
        }
    ...
    public abstract class MetaLanguageConfigBase
    {
        private static MetaLanguageConfig instance = null;
        public static MetaLanguageConfig Instance
        {
            get 
            {
                if (instance == null)
                {
					// If there is a compile error in the following line, create a class MetaLanguageConfig
					// which is a subclass of MetaLanguageConfigBase
                    instance = new MetaLanguageConfig();
                }
                return instance;
            }
        }
        private List<MetaLanguageColorableItem> colorableItems = new List<MetaLanguageColorableItem>();
        public IList<MetaLanguageColorableItem> ColorableItems
        {
        ...
        {
            colorableItems.Add(new MetaLanguageColorableItem(name, type, (COLORINDEX)(-1), COLORINDEX.CI_USERTEXT_BK, foregroundColor, Color.White, bold, strikethrough));
        ...
        {
            colorableItems.Add(new MetaLanguageColorableItem(name, type, foregroundIndex, COLORINDEX.CI_USERTEXT_BK, Color.Black, Color.White, bold, strikethrough));
            return (TokenColor)colorableItems.Count;
        }
        ...
        {
            colorableItems.Add(new MetaLanguageColorableItem(name, type, foregroundIndex, backgroundIndex, foregroundColor, backgroundColor, bold, strikethrough));
            return (TokenColor)colorableItems.Count;
        }
    }
    public class MetaLanguageColorableItem : IVsColorableItem, IVsHiColorItem
    {
        ...
        public MetaLanguageColorableItem(string displayName, TokenType tokenType, COLORINDEX foregroundIndex, COLORINDEX backgroundIndex, Color foregroundColor, Color backgroundColor, bool bold, bool strikethrough)
        {
    ...
    [ComVisible(true)]
    [Guid(MetaLanguageConfig.MetaGeneratorServiceGuid)]
    [ProvideObject(typeof(MetaGeneratorService), RegisterUsing = RegistrationMethod.CodeBase)]
    [CodeGeneratorRegistration(typeof(MetaGeneratorService), MetaLanguageConfig.GeneratorName, "{fae04ec1-301f-11d3-bf4b-00c04f79efbc}", GeneratorRegKeyName = MetaLanguageConfig.FileExtension)]
    [CodeGeneratorRegistration(typeof(MetaGeneratorService), MetaLanguageConfig.GeneratorServiceName, "{fae04ec1-301f-11d3-bf4b-00c04f79efbc}", GeneratorRegKeyName = MetaLanguageConfig.GeneratorName, GeneratesDesignTimeSource = true)]
    public class MetaGeneratorService : VsSingleFileGenerator
    {
		...
		{
			// If there is a compile error in the following line, create a class MetaGenerator
			// which is a subclass of SingleFileGenerator
			return new MetaGenerator(inputFilePath, inputFileContents, defaultNamespace);
		}

        public override string GetDefaultFileExtension()
        {
            return MetaGenerator.DefaultExtension;
        }
    }
    public class MetaLanguageScanner : IScanner
    {
        private int currentOffset;
        private string currentLine;
        private MetaDslx.Languages.Meta.Syntax.InternalSyntax.MetaLexer lexer;
        private MetaDslx.Languages.Meta.Syntax.MetaSyntaxFacts syntaxFacts;
        private Dictionary<LanguageScannerState, int> inverseStates;
        private Dictionary<int, LanguageScannerState> states;
        private int lastState;
        public MetaLanguageScanner()
        {
            this.inverseStates = new Dictionary<LanguageScannerState, int>();
            this.states = new Dictionary<int, LanguageScannerState>();
            this.lastState = 0;
            this.syntaxFacts = MetaDslx.Languages.Meta.Syntax.MetaSyntaxFacts.Instance;
        }
        private void LoadState(int state, MetaDslx.Languages.Meta.Syntax.InternalSyntax.MetaLexer lexer)
        {
            LanguageScannerState value = default(LanguageScannerState);
            this.states.TryGetValue(state, out value);
            value.Restore(lexer);
        }
        private int SaveState(MetaDslx.Languages.Meta.Syntax.InternalSyntax.MetaLexer lexer)
        {
            int result = 0;
        ...
        public bool ScanTokenAndProvideInfoAboutIt(TokenInfo tokenInfo, ref int state)
        {
            try
            {
                if (this.lexer == null)
                {
                    this.lexer = new MetaDslx.Languages.Meta.Syntax.InternalSyntax.MetaLexer(new AntlrInputStream(this.currentLine + "\r\n"));
                }
                ...
                if (tokenType >= 0 && tokenType < MetaLanguageConfig.Instance.ColorableItems.Count)
                {
                    MetaLanguageColorableItem colorItem = MetaLanguageConfig.Instance.ColorableItems[tokenType];
        ...
        {
            public LanguageScannerState(MetaDslx.Languages.Meta.Syntax.InternalSyntax.MetaLexer lexer)
            {
            ...
            public void Restore(MetaDslx.Languages.Meta.Syntax.InternalSyntax.MetaLexer lexer)
            {
    ...
    [ComVisible(true)]
    [Guid(MetaLanguageConfig.MetaLanguageServiceGuid)]
    public class MetaLanguageService : LanguageService
    {
        private LanguagePreferences preferences;
        private MetaLanguageScanner scanner;
        public MetaLanguageService()
        {
			// Creating the config instance
			MetaLanguageConfigBase.Instance.ToString();
        }
        public override string GetFormatFilterList()
        {
            return MetaLanguageConfig.FilterList;
        }
        public override LanguagePreferences GetLanguagePreferences()
        {
            if (preferences == null)
            {
                preferences = new LanguagePreferences(this.Site, typeof(MetaLanguageService).GUID, this.Name);
                preferences.Init();
            }
            return preferences;
        }
        public override IScanner GetScanner(IVsTextLines buffer)
        {
            if (scanner == null)
            {
                scanner = new MetaLanguageScanner();
            }
            return scanner;
        }
        public override Microsoft.VisualStudio.Package.Source CreateSource(IVsTextLines buffer)
        {
            return new MetaLanguageSource(this, buffer, this.GetColorizer(buffer));
        }
        #region Custom Colors
        public override int GetColorableItem(int index, out IVsColorableItem item)
        {
            if (index >= 0 && index < MetaLanguageConfig.Instance.ColorableItems.Count)
            {
                item = MetaLanguageConfig.Instance.ColorableItems[index];
                return Microsoft.VisualStudio.VSConstants.S_OK;
            }
            else
            {
                item = MetaLanguageConfig.Instance.ColorableItems[0];
                return Microsoft.VisualStudio.VSConstants.S_OK;
            }
        }
        public override int GetItemCount(out int count)
        {
            count = MetaLanguageConfig.Instance.ColorableItems.Count;
            return Microsoft.VisualStudio.VSConstants.S_OK;
        }
        #endregion
        public override void OnIdle(bool periodic)
        {
            ...
            MetaLanguageSource src = (MetaLanguageSource)GetSource(this.LastActiveTextView);
        ...
        {
            get { return MetaLanguageConfig.LanguageName; }
        }
        public override ViewFilter CreateViewFilter(CodeWindowManager mgr, IVsTextView newView)
        {
            return new MetaLanguageViewFilter(mgr, newView);
        }
        public override Colorizer GetColorizer(IVsTextLines buffer)
        {
            return new MetaLanguageColorizer(this, buffer, this.GetScanner(buffer));
        }
        public override AuthoringScope ParseSource(ParseRequest req)
        {
			try
			{
				MetaLanguageSource source = (MetaLanguageSource)this.GetSource(req.FileName);
				switch (req.Reason)
				{
					case ParseReason.Check:
                        ...
                        var tree = MetaDslx.Languages.Meta.Syntax.MetaSyntaxTree.ParseText(req.Text);
                        var compilation = MetaDslx.Languages.Meta.MetaCompilation.Create("Meta").AddReferences(metaModelReference).AddSyntaxTrees(tree);
            ...
            return new MetaLanguageAuthoringScope();
        }
    }
	public class MetaLanguageSource : Microsoft.VisualStudio.Package.Source
    {
        public MetaLanguageSource(LanguageService service, IVsTextLines textLines, Colorizer colorizer)
            : base(service, textLines, colorizer)
        {
        }
    }
    public class MetaLanguageViewFilter : ViewFilter
    {
        public MetaLanguageViewFilter(CodeWindowManager mgr, IVsTextView view)
            : base(mgr, view)
        {
        }
