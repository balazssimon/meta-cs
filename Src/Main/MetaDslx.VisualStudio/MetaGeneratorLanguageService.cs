...
namespace MetaDslx.VisualStudio
{
    ...
    public class MetaGeneratorLanguageScanner : IScanner
    {
        ...
        private void LoadState(int state, MetaDslx.Compiler.MetaGeneratorLexer lexer)
        {
            LanguageScannerState value = default(LanguageScannerState);
            this.states.TryGetValue(state, out value);
            value.Restore(lexer);
        }
        private int SaveState(MetaDslx.Compiler.MetaGeneratorLexer lexer)
        {
            int result = 0;
        ...
        public bool ScanTokenAndProvideInfoAboutIt(TokenInfo tokenInfo, ref int state)
        {
            try
            {
                if (this.lexer == null)
                {
                    this.lexer = new MetaDslx.Compiler.MetaGeneratorLexer(new AntlrInputStream(this.currentLine + "\r\n"));
                }
                ...
                if (tokenType >= 1 && tokenType <= MetaGeneratorLanguageConfig.Instance.ColorableItems.Count)
                {
                    MetaGeneratorLanguageColorableItem colorItem = MetaGeneratorLanguageConfig.Instance.ColorableItems[tokenType - 1];
        ...
        {
            public LanguageScannerState(MetaDslx.Compiler.MetaGeneratorLexer lexer)
            {
            ...
            public void Restore(MetaDslx.Compiler.MetaGeneratorLexer lexer)
            {
    ...
    public class MetaGeneratorLanguageService : LanguageService
    {
        ...
        {
            if (index >= 1 && index <= MetaGeneratorLanguageConfig.Instance.ColorableItems.Count)
            {
                item = MetaGeneratorLanguageConfig.Instance.ColorableItems[index - 1];
                return Microsoft.VisualStudio.VSConstants.S_OK;
            }
            else
            {
