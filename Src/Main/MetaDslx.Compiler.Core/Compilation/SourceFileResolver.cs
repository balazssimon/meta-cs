...
namespace MetaDslx.Compiler
{
    ...
    public class SourceFileResolver : SourceReferenceResolver, IEquatable<SourceFileResolver>
    {
            ...
            ImmutableArray<KeyValuePair<string, string>> pathMap)
        {
            ...
            {
                throw new ArgumentException("Absolute path expected.", nameof(baseDirectory));
            }
            ...
            if (!pathMap.IsDefaultOrEmpty)
            {
                foreach (var kv in pathMap)
                {
                    ...
                    {
                        throw new ArgumentException("Empty key in path map.", nameof(pathMap));
                    }

                    var value = kv.Value;
                    if (value == null)
                    {
                        throw new ArgumentException("Null value in path map.", nameof(pathMap));
                    }

                    if (IsPathSeparator(key[key.Length - 1]))
                    {
                        throw new ArgumentException("Key in path map ends with separator.", nameof(pathMap));
                    }
                }
            }
        }
        ...
        {
            FileUtilities.RequireAbsolutePath(resolvedPath, nameof(resolvedPath));
